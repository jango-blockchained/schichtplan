...
def generate_generic_config():
    ...
    config = {
        ...
        "cwd": str(project_root),
        "transports": {
            "stdio": {"command": ["python3", "src/backend/mcp_server.py"]},
            "sse": {
...
def setup_claude():
    ...
    if config_path.exists():
        ...
        print(
            "Please manually merge the Schichtplan MCP server configuration with your existing config."
        )
        print()
...
def main():
    ...
    parser.add_argument(
        "--tool", choices=["claude", "cursor"], help="Specific tool to set up"
    )
    ...
    parser.add_argument(
        "--test", action="store_true", help="Test if MCP server can be started"
    )
