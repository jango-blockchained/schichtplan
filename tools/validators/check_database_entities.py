...
with app.app_context():
    ...
    print(
        "Active Employees:",
        db.session.query(Employee).filter_by(is_active=True).count(),
    )
    ...
    if employees:
        ...
        for employee in employees:
            print(
                f"  {employee.first_name} {employee.last_name} (ID: {employee.id}, Group: {employee.employee_group}, Hours: {employee.contracted_hours})"
            )
    ...
    if shifts:
        ...
        for shift in shifts:
            print(
                f"  ID: {shift.id}, Type: {shift.shift_type}, Time: {shift.start_time}-{shift.end_time}, Active Days: {shift.active_days}"
            )
    ...
    if coverage:
        ...
        for c in coverage:
            print(
                f"  Day: {c.day_index}, Time: {c.start_time}-{c.end_time}, Min Staff: {c.min_employees}, Requires Keyholder: {c.requires_keyholder}"
            )
