...
def remove_min_employees_column():
    """Remove min_employees column from shifts table"""
    ...
    try:
        ...
        if "min_employees" in columns:
            # In SQLite, removing a column requires recreating the table
            # 1. Get current table schema
            cursor.execute("PRAGMA table_info(shifts)")
            columns_info = cursor.fetchall()

            # Filter out the min_employees column
            columns_to_keep = [col for col in columns_info if col[1] != "min_employees"]

            # 2. Create a new table without the min_employees column
            ...
            conn.commit()
            print("Successfully removed min_employees column from shifts table")
        else:
            print("Column min_employees does not exist in shifts table")
