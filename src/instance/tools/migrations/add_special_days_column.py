...
# Parse arguments
parser = argparse.ArgumentParser(description="Add special_days column to settings table")
...
instance_dir = tools_dir.parent       # /src/instance
...
def main():
    ...
    try:
        ...
        if args.dry_run:
            ...
            cursor.execute("ALTER TABLE settings ADD COLUMN special_days TEXT DEFAULT '{}'")
            ...
            for row in rows:
                ...
                if special_hours_json:
                    try:
                        ...
                        print(f"Converted {len(special_days)} special hours to special days format")
                        
                    except json.JSONDecodeError:
                        print(f"Warning: Invalid JSON in special_hours for settings ID {settings_id}")
            ...
            if 'special_days' in columns:
                ...
                print("WARNING: Verification failed! special_days column not found after migration")
