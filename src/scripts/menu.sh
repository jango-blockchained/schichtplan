...
# Function to clear a tmux pane
clear_pane() {
    local pane=$1
    tmux send-keys -t schichtplan:0.$pane C-l
}
...
restart_frontend() {
    ...
    sleep 1
    tmux send-keys -t schichtplan:0.1 "tail -f ../logs/tmux_frontend_output.log" C-m
...
restart_all() {
    restart_backend
    restart_frontend
    echo "Both backend and frontend restarted."
}
...
close_tmux_session() {
    ...
    echo "Session closed. Press any key to exit this pane."
    read -n 1 -s
    exit 0
}
...
# Note: MCP server is automatically managed by VS Code
# VS Code spins up and reloads MCP server instances automatically on changes
