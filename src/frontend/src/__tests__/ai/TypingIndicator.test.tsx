...
describe('TypingIndicator Component', () => {
  ...
  test('handles empty users array', () => {
    const { container } = render(
      <TypingIndicator typing={true} users={[]} />
    );
  ...
  test('has proper accessibility attributes', () => {
    ...
    expect(indicator?.getAttribute('aria-live') || 
           indicator?.getAttribute('role') ||
           indicator?.getAttribute('aria-label')).toBeTruthy();
  });
  ...
  test('displays user count correctly', () => {
    ...
    expect(users.some(user => text.includes(user)) || text.includes('3')).toBe(true);
  });
