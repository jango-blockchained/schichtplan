...
describe('FileUploadComponent', () => {
  ...
  test('shows maximum file size', () => {
    ...
    const { container } = render(
      <FileUploadComponent maxFileSize={maxSize} />
    );
  ...
  test('shows maximum file count', () => {
    const maxFiles = 3;
    const { container } = render(
      <FileUploadComponent maxFiles={maxFiles} />
    );
  ...
  test('provides visual drag feedback', () => {
    ...
    if (uploadArea) {
      ...
      expect(uploadArea.classList.contains('drag-over') || 
             uploadArea.classList.contains('dragging')).toBe(true);
    }
  });
  ...
  test('is accessible', () => {
    ...
    expect(uploadArea?.getAttribute('tabIndex') ||
           uploadArea?.getAttribute('role') ||
           uploadArea?.getAttribute('aria-label')).toBeTruthy();
  });
  ...
  test('handles upload cancellation', async () => {
    ...
    if (fileInput) {
      ...
      const cancelButton = container.querySelector('[data-testid="cancel-upload"]') ||
