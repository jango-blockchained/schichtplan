...
const createMockFn = () => {
  ...
  fn.toHaveBeenCalledWith = (expectedArgs: any) => 
    fn.calls.some(call => JSON.stringify(call) === JSON.stringify(expectedArgs));
  return fn;
};
...
describe('VoiceInput Component', () => {
  ...
  test('disables microphone button when disabled', () => {
    const { container } = render(<VoiceInput enabled={false} />);
  ...
  test('handles speech recognition results', async () => {
    ...
    const { container } = render(<VoiceInput enabled={true} onTranscription={onTranscription} />);
    ...
    await act(async () => {
      ...
      const addEventListenerCalls = mockSpeechRecognition.addEventListener.calls;
      const resultCallback = addEventListenerCalls.find(call => call[0] === 'result');
      if (resultCallback) {
  ...
  test('shows audio level visualization when recording', async () => {
    const { container } = render(<VoiceInput enabled={true} showAudioLevel={true} />);
  ...
  test('handles voice command processing', async () => {
    ...
    const { container } = render(<VoiceInput enabled={true} onCommand={onCommand} />);
    ...
    await waitFor(() => {
      expect(mockAiService.processVoiceCommand.toHaveBeenCalledWith('create schedule for next week')).toBe(true);
    });
  });

  test('handles microphone permission errors', async () => {
    const onError = createMockFn();
    const { container } = render(<VoiceInput enabled={true} onError={onError} />);
    ...
    await act(async () => {
      ...
      const addEventListenerCalls = mockSpeechRecognition.addEventListener.calls;
      const errorCallback = addEventListenerCalls.find(call => call[0] === 'error');
      if (errorCallback) {
        errorCallback[1](errorEvent);
      }
    });
    
    expect(onError.toHaveBeenCalledWith('Microphone permission denied')).toBe(true);
  });

  test('supports different languages', () => {
    const { container } = render(<VoiceInput enabled={true} language="de-DE" />);
  ...
  test('handles continuous recording mode', () => {
    const { container } = render(<VoiceInput enabled={true} continuous={true} />);
  ...
  test('cleans up on unmount', () => {
    const { unmount } = render(<VoiceInput enabled={true} />);
    
    unmount();
    
    // Should clean up without errors
    expect(mockSpeechRecognition.removeEventListener.toHaveBeenCalled()).toBe(true);
  });

  test('shows confidence score for speech recognition', async () => {
    const { container } = render(<VoiceInput enabled={true} showConfidence={true} />);
    ...
    const confidenceIndicator = container.querySelector('[data-testid="confidence-indicator"]');
  ...
  test('handles speech recognition errors gracefully', async () => {
    ...
    expect(onError.toHaveBeenCalledWith('Speech recognition error: network')).toBe(true);
  });
