...
export const MainLayout = () => {
  ...
  const currentSidebarWidth = isSidebarMinimized && !isSidebarHovered ? shrunkSidebarWidth : expandedSidebarWidth;

  const NavItem = ({ item, isActive, minimized }: { item: { label: string; path: string; icon: React.ElementType }; isActive: boolean; minimized: boolean }) => {
    ...
    return (
      <div>
        <Button
          ...
          asChild
        >
          <RouterLink to={item.path} title={item.label}> {/* Add title for tooltip effect on hover */}
  ...
  const NavContent = ({ minimized }: { minimized: boolean }) => (
    <div className="h-full flex flex-col">
      ...
      <nav className="flex-1 p-2"> {/* Adjusted padding for minimized state */}
        {mainNavItems.map((item) => {
          const isActive = location.pathname === item.path;
          return <NavItem key={item.path} item={item} isActive={isActive} minimized={minimized} />;
        })}
      </nav>

      {/* Footer navigation items */}
      <div className="p-2 border-t"> {/* Adjusted padding */}
    ...
    <div className="min-h-screen bg-background">
      ...
      <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
        ...
        <SheetContent side="left" className="w-[240px] p-0">
          <NavContent minimized={false} /> {/* Mobile drawer is never "minimized" in this sense */}
        </SheetContent>
      </Sheet>
