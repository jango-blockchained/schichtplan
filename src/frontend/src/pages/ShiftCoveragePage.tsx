          ...
          <div className="relative w-96 h-96 mx-auto">
            ...
            {timeSlots.map((slot, index) => {
              ...
              return (
                <div
                  key={slot.time}
                  className={`absolute flex items-center justify-center w-8 h-8 -ml-4 -mt-4 rounded-full ${
              ...
              <div key={slot.time} className="flex items-center space-x-2">
                <div
                  className={`w-4 h-4 rounded-full ${
                    slot.shifts.length > 0 ? "bg-blue-500" : "bg-gray-100"
                  }`}
                />
                <span>
                  {slot.time} - {slot.shifts.length} shifts
                </span>
              </div>
            ))}
          </div>
