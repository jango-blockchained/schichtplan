 ...
 */
export function getISOWeekInfo(
  targetDate: Date,
 ...
 */
export function getWeekFromIdentifier(
 ...
 */
export function getCurrentWeekIdentifier(opts?: {
  weekStartsOn?: 0 | 1;
}): string {
 ...
 */
export function dateRangeToWeekIdentifier(
  startDate: Date,
  endDate: Date,
  opts?: { weekStartsOn?: 0 | 1 },
): string {
  ...
  if (
    ...
    startWeek.weekNumber === endWeek.weekNumber
  ) {
 ...
 */
export async function getDynamicWeekStartsOn(
  queryClient?: QueryClient,
): Promise<0 | 1> {
  try {
    if (queryClient) {
      const cached = queryClient.getQueryData<Settings | undefined>([
        "settings",
      ]);
