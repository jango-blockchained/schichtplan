      ...
      try {
        ...
        const timeout = setTimeout(() => {
          addGenerationLog(
            "error",
            "Zeitüberschreitung",
            "Die Generierung dauert länger als erwartet. Bitte versuchen Sie es erneut.",
          );
          ...
          try {
            ...
            console.log("✅ GenerateSchedule API response:", {
              ...
              "Schedules with shifts":
                result.schedules?.filter((s) => s.shift_id !== null)?.length ||
                0,
              "Unique employees": [
            ...
            if (result.logs && result.logs.length > 0) {
              ...
              const formattedLogs: GenerationLog[] = result.logs.map(
                (log: any) => ({
                  timestamp: log.timestamp || new Date().toISOString(),
                  type: (log.level?.toLowerCase() || "info") as
                    | "info"
                    | "warning"
                    | "error",
            ...
            if (apiError instanceof Error) {
              ...
              if (hasDurationError) {
                addGenerationLog(
                  "error",
                  "Schichtdauer fehlt",
                  "Bitte überprüfen Sie die Schichteinstellungen und stellen Sie sicher, dass alle Schichten eine Dauer haben.",
                );
              }
            } else {
              addGenerationLog(
    ...
    onError: (error: unknown) => {
      ...
      if (axios.isAxiosError(error) && error.response?.data?.logs) {
        backendLogs = error.response.data.logs.map((log: any) => ({
          timestamp: log.timestamp || new Date().toISOString(),
          type: (log.level?.toLowerCase() || "error") as
            | "info"
            | "warning"
            | "error",
          message: log.message || "Log message missing",
