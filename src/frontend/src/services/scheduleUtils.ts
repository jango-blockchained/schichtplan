...
export async function validateConsecutiveShiftRequirements(
  ...
  const requirements = getConsecutiveShiftRequirements(
    shift,
    date,
    openingDays,
  );
  ...
  try {
    ...
    for (const requirement of requirements) {
      ...
      if (existingSchedule && !existingSchedule.is_empty) {
        ...
        const existingShiftType = isEarlyShift(existingSchedule)
          ? "EARLY"
          : isLateShift(existingSchedule)
            ? "LATE"
            : "MIDDLE";
  ...
  try {
    ...
    for (const requirement of requirements) {
      ...
      if (!existingSchedule || existingSchedule.is_empty) {
        ...
        const shiftTemplate = shifts.find(
          (s) =>
        ...
        if (shiftTemplate) {
          logService.info(
            "scheduleUtils",
            "create-consecutive-shift",
            `Creating ${requirement.shiftType} shift for employee ${employeeId} on ${requirement.date} (${requirement.reason})`,
          );
          ...
          console.warn(
            `No ${requirement.shiftType} shift template found for consecutive shift requirement`,
          );
        }
      }
    }
    ...
    throw new Error(
      `Failed to create required consecutive shifts: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
