...
const DataManagementSection: React.FC = () => {
    ...
    const fetchTables = async () => {
      ...
      try {
        ...
        toast({
          ...
          description: error instanceof Error ? error.message : "Could not load table list.",
          variant: "destructive",
        });
  ...
  const handleAction = async (action: () => Promise<any>, successMessage: string, errorMessagePrefix: string) => {
    setIsProcessing(true);
    try {
      await action();
      toast({ title: "Success", description: successMessage });
    } catch (err) {
      toast({ title: "Error", description: `${errorMessagePrefix}: ${err instanceof Error ? err.message : "Unknown error"}`, variant: "destructive" });
    }
  ...
  const handleBackup = async () => {
    ...
    try {
      ...
      toast({ title: "Error", description: `Failed to backup database: ${err instanceof Error ? err.message : "Unknown error"}`, variant: "destructive" });
    }
    setIsProcessing(false);
  };
  ...
  const handleRestoreChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    await handleAction(() => restoreDatabase(file), "Database restored successfully.", "Failed to restore database");
    if(fileInputRef.current) fileInputRef.current.value = "";
  };

  const handleWipeConfirm = async () => {
    if (selectedTablesToWipe.length === 0) {
        toast({ title: "No tables selected", description: "Please select at least one table to wipe.", variant: "warning" });
        return;
    }
    // Close dialog logic will be handled by AlertDialog's onOpenChange or by controlling its open prop
    await handleAction(() => wipeTables(selectedTablesToWipe), "Selected tables wiped successfully.", "Failed to wipe tables");
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center space-x-4">
            <Label htmlFor="demo-data-module" className="min-w-max">Select Module:</Label>
            <Select value={selectedDemoModule} onValueChange={setSelectedDemoModule} disabled={isProcessing}>
        ...
        <CardContent className="space-y-4">
          <div className="flex flex-wrap gap-2">
            <Button onClick={handleBackup} disabled={isProcessing}>
              {isProcessing && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Backup Database
            </Button>
            <input type="file" accept=".json" onChange={handleRestoreChange} ref={fileInputRef} style={{ display: 'none' }} id="restore-file-input" />
            <Button onClick={() => fileInputRef.current?.click()} disabled={isProcessing}> 
              {isProcessing && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Restore Database
            </Button>

            <AlertDialog>
              <AlertDialogTrigger asChild>
                <Button variant="destructive" disabled={isProcessing || isFetchingTables}>
                    {isProcessing && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Wipe Tables
                </Button>
              </AlertDialogTrigger>
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    This action cannot be undone. This will permanently delete the selected tables.
                    Select tables to wipe:
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <div className="max-h-60 overflow-y-auto space-y-2 my-4 p-2 border rounded-md">
                    {isFetchingTables && <div className="flex items-center justify-center"><Loader2 className="mr-2 h-4 w-4 animate-spin" />Loading tables...</div>}
                    {!isFetchingTables && availableTables.length === 0 && <p className="text-sm text-muted-foreground">No tables available or failed to load.</p>}
                    {!isFetchingTables && availableTables.length > 0 && availableTables.map(table => (
                        <div key={table} className="flex items-center space-x-2">
                            ...
                            />
                            <Label htmlFor={`wipe-${table}`} className="font-normal capitalize cursor-pointer">{table.replace(/_/g, ' ')}</Label>
                        </div>
                    ))}
                </div>
                <AlertDialogFooter>
                  ...
                  <AlertDialogAction 
                    ...
                    disabled={isProcessing || selectedTablesToWipe.length === 0 || isFetchingTables}
                    className="bg-destructive hover:bg-destructive/90 focus-visible:ring-destructive"
                  >
                    {isProcessing && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Wipe Selected Tables
                  </AlertDialogAction>
                </AlertDialogFooter>
              </AlertDialogContent>
            </AlertDialog>
