...
const ShiftCell = ({
  ...
  // Validation checks
  const { hasBreakViolation, hasLongBreakViolation, hasHoursViolation } =
    validateBreaks({
  ...
  const shiftTypeInfo = activeSettings?.shift_types?.find(
...
export const ShiftTable = ({
  ...
  const filteredDates =
    filterOpeningDays && activeSettings?.general?.opening_days
      ? dates.filter((date) => {
                ...
                >
                  ...
                  {daysToShow.map((day, dayIndex) => {
                    ...
                    return (
                      <Droppable
                        key={dayOfWeek}
                        droppableId={dayOfWeek.toString()}
                      >
                        {(provided, snapshot) => (
                          <TableCell
