...
export const parseTime = (timeStr: string): Date => parse(timeStr, 'HH:mm', new Date());
...
export const calculateShiftHours = (shift: { 
    ...
    let totalHours = timeToMinutes(shift.end_time) - timeToMinutes(shift.start_time);
    ...
    if (shift.break) {
        let breakDuration = timeToMinutes(shift.break.end) - timeToMinutes(shift.break.start);
        if (breakDuration < 0) {
            breakDuration += 24 * 60;
        }
        ...
        if (shift.break.notes?.includes('Second break:')) {
            ...
            if (secondBreakMatch) {
                ...
                let secondBreakDuration = timeToMinutes(secondBreakEnd) - timeToMinutes(secondBreakStart);
 ...
 */
export const calculateWeeklyHours = (shifts: Array<{ 
    start_time?: string; 
    end_time?: string; 
    break?: { 
        start: string; 
 ...
 */
export const calculateMonthlyHours = (shifts: Array<{ 
    start_time?: string; 
    end_time?: string; 
    break?: { 
        start: string; 
 ...
 */
export const determineShiftType = (startTime: string | undefined): 'EARLY' | 'MIDDLE' | 'LATE' => {
...
export const validateBreaks = (shift: { 
    ...
    const hasLongBreakViolation = shiftMinutes > 540 && (!shift.break?.notes?.includes('Second break:')); // 9 hours = 540 minutes
