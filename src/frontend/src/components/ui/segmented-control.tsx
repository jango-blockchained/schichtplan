...
function SegmentedControlImpl({ value, onChange, className, children }: SegmentedControlProps) {
    ...
    return (
        <div className={`inline-flex rounded-md overflow-hidden ${className || ""}`} role="tablist">
            {items.map((child) => {
...
const SegmentedControl = SegmentedControlImpl as typeof SegmentedControlImpl & {
    Item: (props: { children: React.ReactNode; value?: string | number }) => React.ReactNode;
};

SegmentedControl.Item = ({ children }: { children: React.ReactNode }) => children;
