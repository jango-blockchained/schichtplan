...
import {
  ...
  type SpecialDaysMap,
} from "@/components/UnifiedSettingsSections/SpecialDaysManagement";
...
const HolidayManagement: React.FC = () => {
  ...
  const settings = queryClient.getQueryData(["settings"]) as
    | Settings
    | undefined;
  ...
  const { data: statsData } = useQuery({
    ...
    queryFn: async () => {
      const response = await api.get(
        `/api/holiday/statistics?year=${selectedYear}`,
      );
      return response.data;
    },
  });
  ...
  const previewHolidays = async () => {
    try {
      const response = await api.get(
        `/api/holiday/preview/${selectedYear}${selectedState ? `?state=${selectedState}` : ""}`,
      );
  ...
  const importMutation = useMutation({
    mutationFn: async (force: boolean = false) => {
      const response = await api.post(
        `/api/holiday/import/german/${selectedYear}${selectedState ? `?state=${selectedState}` : ""}${force ? "&force=true" : ""}`,
      );
      return response.data;
    },
    onSuccess: (data) => {
  ...
  const states: FederalState[] = statesData?.states
    ? Object.entries(statesData.states).map(([code, name]) => ({
        code,
        name: name as string,
      }))
    : [];
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <SpecialDaysManagement
            ...
            onImmediateUpdate={() => {
              const current = queryClient.getQueryData(["settings"]) as
                | Settings
                | undefined;
              if (!current) return;
          ...
          <CardContent>
            <div className="mb-4">
              <p className="text-sm text-muted-foreground">
                ...
                {selectedState &&
                  ` in ${states.find((s) => s.code === selectedState)?.name}`}
              </p>
            </div>

            <div className="space-y-4">
              {Object.entries(previewData.holidays_by_month).map(
                ([month, holidays]: [string, HolidayPreview[]]) => (
                  <div key={month}>
                      ...
                      {holidays.map((holiday: HolidayPreview) => (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <Badge
                              variant={
                                holiday.is_closed ? "destructive" : "secondary"
                              }
                            >
      ...
      {importMutation.data?.conflicts && (
        <Alert>
          <AlertCircle className="h-4 w-4" />
          <AlertDescription>
            Found {importMutation.data.conflicts.length} existing holidays. Use
            "Force Import" to overwrite them.
          </AlertDescription>
        </Alert>
      )}
    </div>
