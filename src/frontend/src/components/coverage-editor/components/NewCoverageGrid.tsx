...
const generateTimeSlots = (
  ...
  if (storeConfig.keyholder_before_minutes) {
    ...
    beforeTime.setMinutes(
      beforeTime.getMinutes() - storeConfig.keyholder_before_minutes,
    );
    beforeSlot = beforeTime.toLocaleTimeString("de-DE", {
      hour: "2-digit",
      minute: "2-digit",
    });
  ...
  while (current <= storeEnd) {
    const timeSlot = current.toLocaleTimeString("de-DE", {
      hour: "2-digit",
      minute: "2-digit",
    });
    slots.push(timeSlot);
  ...
  if (storeConfig.keyholder_after_minutes) {
    ...
    afterTime.setMinutes(
      afterTime.getMinutes() + storeConfig.keyholder_after_minutes,
    );
    afterSlot = afterTime.toLocaleTimeString("de-DE", {
      hour: "2-digit",
      minute: "2-digit",
    });
...
export const NewCoverageGrid: React.FC<NewCoverageGridProps> = ({
  ...
  const { allSlots, regularSlots, beforeSlot, afterSlot } =
    generateTimeSlots(storeConfig);
                    ...
                    >
                      ...
                      >
                        ...
                        <span className="text-xs font-medium whitespace-nowrap overflow-hidden">
                          {slot.startTime}-{slot.endTime} ({slot.minEmployees}-
                          {slot.maxEmployees})
                        </span>
                      </div>
                    </div>
