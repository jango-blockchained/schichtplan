...
export const AIEmployeeInsights: React.FC<AIEmployeeInsightsProps> = ({
    ...
    const [selectedInsight, setSelectedInsight] = useState<WorkloadInsight | null>(null);
    ...
    const fetchInsights = async () => {
        ...
        try {
            ...
            const workloadLevels: Array<"underutilized" | "balanced" | "overworked"> = ["underutilized", "balanced", "overworked"];
            ...
            if (selectedEmployeeId) {
                const insight = mockInsights.find(i => i.employee_id === selectedEmployeeId);
    ...
    const handleSuggestAvailability = async () => {
        try {
            ...
            await enhancedAIService.suggestAvailability(aiContext.getContextSummary());
    ...
    const summary = React.useMemo(() => {
        ...
        const overworked = insights.filter(i => i.workload_level === "overworked").length;
        const underutilized = insights.filter(i => i.workload_level === "underutilized").length;
        const balanced = insights.filter(i => i.workload_level === "balanced").length;
        const avgHours = insights.reduce((sum, i) => sum + i.avg_hours_per_week, 0) / insights.length;
        const avgAvailability = insights.reduce((sum, i) => sum + i.availability_score, 0) / insights.length;
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                        ...
                                        <CardContent className="p-4">
                                            <div className="flex items-center justify-between">
                                                <div>
                                                    ...
                                                    <p className="text-2xl font-bold">{summary.balanced}</p>
                                                </div>
                                    ...
                                    <Card>
                                        <CardContent className="p-4">
                                            <div className="flex items-center justify-between">
                                                <div>
                                                    <p className="text-xs text-muted-foreground">Overworked</p>
                                                    <p className="text-2xl font-bold">{summary.overworked}</p>
                                                </div>
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div>
                                        <div className="flex items-center justify-between text-sm mb-1">
                                            ...
                                            <span className="font-medium">{summary.avgAvailability}%</span>
                                        </div>
                            ...
                            <ScrollArea className="h-[400px] pr-4">
                                <div className="space-y-3">
                                    {insights.map((insight) => {
                                        const WorkloadIcon = workloadLevelIcons[insight.workload_level];
                                        ...
                                        return (
                                            <Card
                                                ...
                                                className={cn(
                                                    ...
                                                    selectedInsight?.employee_id === insight.employee_id && "ring-2 ring-purple-500"
                                                )}
                                                    ...
                                                    <div className="flex items-start justify-between gap-3">
                                                        <div className="flex-1">
                                                            <div className="flex items-center gap-2 mb-2">
                                                                ...
                                                                <Badge
                                                                    variant="outline"
                                                                    className={cn("h-5 text-xs", workloadLevelColors[insight.workload_level])}
                                                                >
                                                            ...
                                                            <div className="grid grid-cols-2 gap-2 text-xs text-muted-foreground">
                                                                <div className="flex items-center gap-1">
                                                                    <Clock className="h-3 w-3" />
                                                                    <span>{insight.avg_hours_per_week}h/week</span>
                                                                </div>
                                                                <div className="flex items-center gap-1">
                                                                    <Calendar className="h-3 w-3" />
                                                                    <span>{insight.consecutive_days} consecutive days</span>
                                                                </div>
                                                            </div>

                                                            {selectedInsight?.employee_id === insight.employee_id && (
