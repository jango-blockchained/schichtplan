    ...
    const generateOptimization = useCallback(async () => {
        ...
        try {
            ...
            const response = await aiService.optimizeScheduleWithAI({
                ...
                optimization_goals: ['minimize_conflicts', 'improve_coverage', 'balance_workload'],
                constraints: {
            ...
            if (response.success) {
                ...
                const mockSuggestions: OptimizationSuggestion[] = response.recommendations.map((rec, index) => ({
                    id: `opt-${index}`,
                ...
                setMetrics({
                    totalConflicts: response.improvements.find(imp => imp.metric.includes('conflict'))?.before || 0,
                    coverageGaps: response.improvements.find(imp => imp.metric.includes('coverage'))?.before || 0,
                    workloadImbalance: response.improvements.find(imp => imp.metric.includes('workload'))?.before || 0,
            ...
            <CardContent className="pt-0">
                ...
                <div className="mb-4">
                    <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-xs text-muted-foreground">{metrics.efficiency}%</span>
                    </div>
                            ...
                            suggestions.map((suggestion) => (
                                ...
                                >
                                    ...
                                    <div className="mb-2 p-2 bg-background rounded text-xs">
                                        <div className="font-medium mb-1">{suggestion.impact.metric}</div>
                                        <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div className="text-xs text-muted-foreground">
                                            {suggestion.timeframe} • {Math.round(suggestion.confidence * 100)}% confidence
                                        </div>
