...
export const FileUploadComponent: React.FC<FileUploadComponentProps> = ({
    ...
    const getFileIcon = (type: string) => {
        ...
        if (type.includes('text') || type.includes('plain')) return <FileText className="h-4 w-4" />;
        ...
        try {
            for (let i = 0; i < files.length; i++) {
                ...
                try {
                    ...
                    if (file.type.includes('csv') || file.type.includes('json') || file.type.includes('spreadsheet')) {
                        try {
