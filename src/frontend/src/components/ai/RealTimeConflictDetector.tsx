...
export interface ScheduleConflict {
    id: string;
    type: 'overlap' | 'double_booking' | 'insufficient_rest' | 'skill_mismatch' | 'capacity_exceeded';
...
export const RealTimeConflictDetector: React.FC<RealTimeConflictDetectorProps> = ({
    className,
    autoRefresh = true,
    ...
    const detectConflicts = useCallback(async () => {
        ...
        try {
            ...
            const mockConflicts: ScheduleConflict[] = [
                {
                    ...
                    description: 'Sarah Johnson is scheduled for overlapping shifts on Friday 2-6 PM and 4-8 PM',
                ...
                },
                {
                    ...
                    description: 'Mike Davis has less than 8 hours rest between shifts on Thursday',
                    affectedEmployees: ['Mike Davis'],
                    affectedShifts: ['shift-125', 'shift-126'],
                    suggestedResolution: 'Add buffer time or reschedule one of the shifts',
                    timestamp: new Date(),
                    resolved: false
                },
                {
                    ...
                    description: 'Kitchen department exceeds maximum capacity during lunch rush',
            ...
            setConflicts(prev => {
                ...
                const newConflicts = mockConflicts.filter(mock =>
                    !unresolved.some(existing => existing.id === mock.id)
                );
            ...
            <CardHeader className="pb-3">
                ...
                <div className="flex items-center gap-4 text-xs">
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        <span>Medium: {activeConflicts.filter(c => c.severity === 'medium').length}</span>
                    </div>
                    <div className="flex items-center gap-1">
                        <div className="w-2 h-2 bg-blue-500 rounded-full" />
                        <span>Low: {activeConflicts.filter(c => c.severity === 'low').length}</span>
                    </div>
                </div>
            </CardHeader>
                                ...
                                >
                                    ...
                                    {conflict.suggestedResolution && (
                                        <div className="mb-2 p-2 bg-blue-50 border border-blue-200 rounded text-xs">
                                            ...
                                            <span className="text-blue-700">{conflict.suggestedResolution}</span>
                                        </div>
                                    )}
