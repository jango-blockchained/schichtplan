...
export function ScheduleOverview({
  ...
  const shiftsWithBreaks = schedules.filter(
    (s) => s.break_start && s.break_end,
  ).length;
  const shiftsWithNotes = schedules.filter((s) => s.notes).length;

  // Group schedules by employee type
  const employeeTypeStats = schedules.reduce(
    (acc, schedule) => {
      const type = schedule.employee_name
        ? (schedule.employee_name.match(/\((.*?)\)/)?.[1] ?? "Other")
        : "Other";
    ...
    <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="mt-2 text-xs">
            {((shiftsWithBreaks / totalShifts) * 100).toFixed(1)}% aller
            Schichten
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="mt-2 text-xs">
            {((shiftsWithNotes / totalShifts) * 100).toFixed(1)}% aller
            Schichten
          </div>
        </CardContent>
      </Card>
