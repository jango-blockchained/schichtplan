...
export function AIConversationGenerationDialog({
  ...
  const [currentStep, setCurrentStep] =
    useState<ConversationStep>("initialize");
    ...
    try {
      ...
      setError(
        error instanceof Error
          ? error.message
          : "Failed to analyze current state",
      );
      setIsLoading(false);
    }
  };
  ...
  const generateSchedule = async () => {
    ...
    try {
      ...
      const errMsg =
        error instanceof Error
          ? error.message
          : (error && (error as any).message) || "Failed to generate schedule";
      setError(errMsg);
      setIsLoading(false);
      ...
      if (
        errObj?.error_code === "MISSING_GEMINI_API_KEY" ||
        (errObj?.message && errObj.message.includes("Gemini API key"))
      ) {
        toast.error(
          "AI generation requires a Gemini API key. Please configure it in your environment settings.",
        );
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {recommendations.focus_areas?.map(
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    {Object.entries(
                      recommendations.optimization_parameters
                        ?.priority_settings || {},
                    ).map(([key, value]) => (
                      <div
                        key={key}
              ...
              <div className="space-y-4">
                <Alert className="border-green-200 bg-green-50 dark:bg-green-950">
                  ...
                  <AlertDescription>
                    ...
                    {generationResult.generated_assignments_count || 0}{" "}
                    assignments
                  </AlertDescription>
                </Alert>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium">
                      {generationResult.version || "N/A"}
                    </span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-muted-foreground">Session ID:</span>
                    <span className="font-mono text-xs">
                      {generationResult.session_id || "N/A"}
                    </span>
                  </div>
      ...
      case "complete":
        return (
          <div className="text-center py-8">
            ...
            <p className="text-muted-foreground">
              The optimized schedule has been saved.
            </p>
          </div>
        );
      ...
      <DialogContent className="max-w-3xl max-h-[80vh]">
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            {(Object.keys(STEP_DETAILS) as ConversationStep[]).map(
              (step, index) => {
