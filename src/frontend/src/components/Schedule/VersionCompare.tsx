...
export function VersionCompare({
  ...
  const changes = useMemo(() => {
    ...
    currentVersion.forEach((current) => {
      const previous = previousVersion.find(
        (p) => p.date === current.date && p.shift_id === current.shift_id,
      );
      ...
      if (!previous) {
        ...
        if (
          previous.break_start !== current.break_start ||
          previous.break_end !== current.break_end
        ) {
          fieldChanges.push({
            field: "break",
            old:
              previous.break_start && previous.break_end
                ? `${previous.break_start}-${previous.break_end}`
                : "None",
            new:
