      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <div className="text-center">
            ...
            <div className="text-xs text-muted-foreground">Gesamt Schichten</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold">{avgHoursPerWeek.toFixed(1)}</div>
          ...
          <div className="text-center">
            ...
            <div className="text-xs text-muted-foreground">Ø Schichten/Woche</div>
          </div>
        </div>
            ...
            return (
              <div key={index} className="p-3 border rounded-lg space-y-2">
                ...
                <div className="flex justify-between text-xs text-muted-foreground pt-1">
                  <span>Ø {(week.hours / Math.max(week.shifts, 1)).toFixed(1)}h pro Schicht</span>
                  <span>Ø {(week.hours / Math.max(week.employees, 1)).toFixed(1)}h pro MA</span>
                </div>
              </div>
            );
          })}
          ...
          <div className="border-t pt-3">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <div className="font-medium">
                  KW {weeklyBreakdown.find(w => w.hours === Math.min(...weeklyBreakdown.map(w => w.hours)))?.weekNumber}
                  ({Math.min(...weeklyBreakdown.map(w => w.hours)).toFixed(1)}h)
                </div>
              </div>
            </div>
          </div>
