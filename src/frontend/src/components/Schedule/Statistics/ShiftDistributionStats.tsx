...
import { BarChart3, Moon, PieChart, Sun, Sunrise } from "lucide-react";
import {
  Bar,
  BarChart,
  CartesianGrid,
...
export function ShiftDistributionStats({
  ...
  const pieData = SHIFT_DATA.map((item) => ({
    ...
    percentage:
      totalSchedules > 0
        ? (shiftTypeStats[item.key as keyof typeof shiftTypeStats] /
            totalSchedules) *
          100
        : 0,
  }));
              ...
              <RechartsPieChart>
                ...
                <Legend
                  ...
                  formatter={(value) => (
                    <span
                      style={{
                        color: pieData.find((d) => d.name === value)?.color,
                      }}
                    >
                      {value}
                    </span>
              ...
              >
                ...
                <Tooltip
                  formatter={(value: number, name: string) => {
                    ...
                    if (name === "percentage")
                      return [`${value.toFixed(1)}%`, "Prozent"];
                    return [value, name];
                  }}
                />
                <Bar
