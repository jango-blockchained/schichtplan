...
export function ClassicAIGenerationDialog({
    ...
    const conversation = useAIConversation(async (prompt: string) => {
        ...
        // TODO [AI Team, Q3 2024]: Parse prompt and adjust options accordingly.
        // Currently, free-text instructions are not mapped to structured options.
        await handleConfirm();
        return { message: "Anweisung wurde in die Generierung integriert." };
    });

    const handlePriorityChange = (
        key: keyof ClassicAIOptions["prioritySettings"],
        value: number[]
    ) => {
        setOptions((prev) => ({
            ...prev,
    ...
    const handleConstraintToggle = (
        key: keyof ClassicAIOptions["constraintOverrides"],
        checked: boolean
    ) => {
        setOptions((prev) => ({
            ...prev,
    ...
    const handleEmployeeOptionToggle = (
        key: keyof ClassicAIOptions["employeeOptions"],
        checked: boolean
    ) => {
        setOptions((prev) => ({
            ...prev,
    ...
    const handleAIParamChange = (
        key: keyof ClassicAIOptions["aiModelParams"],
        value: number[]
    ) => {
        setOptions((prev) => ({
            ...prev,
    ...
    const handleConfirm = async () => {
        ...
        try {
            // Convert classic options to AI generation request
            const generationRequest = {
                ...
                options: {
                    ...
                    employee_options: options.employeeOptions,
                    ai_model_params: options.aiModelParams,
                },
            };

            // Call AI service for generation
            const response = await aiService.generateSchedule(generationRequest);

            if (response.success) {
                toast.success("Schichtplan erfolgreich generiert!");
                onComplete();
                onClose();
            } else {
                throw new Error(response.error || "Fehler bei der Generierung");
            }
        } catch (error) {
    ...
    const handleExportConversationToOptions = () => {
        // Hinweis: Die Übertragung der Unterhaltung in Optionen ist noch nicht implementiert.
        setActiveTab("priorities");
        conversation.addSystemMessage("Die Übertragung der Unterhaltung in strukturierte Optionen ist derzeit noch nicht verfügbar.");
    };
};
        ...
        <DialogContent className="sm:max-w-[900px] max-h-[90vh] overflow-y-auto">
            <DialogHeader>
                <DialogTitle className="flex items-center gap-2">
                    ...
                    Klassische KI-Generierung
                </DialogTitle>
                <DialogDescription>
                    Konfigurieren Sie die detaillierten Optionen für die KI-gestützte Schichtplanerstellung von {startDate} bis {endDate}.
                </DialogDescription>
            </DialogHeader>
                ...
                <Button onClick={handleConfirm} disabled={isGenerating}>
                    {isGenerating ? (
                        ...
                        <>
                            ...
                            Klassische Generierung starten
                        </>
                    )}
                </Button>
            </DialogFooter>
