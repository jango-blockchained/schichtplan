...
export function ScheduleMetricsCards({
  ...
  const utilization =
    totalEmployees > 0 ? (uniqueScheduledEmployees / totalEmployees) * 100 : 0;
  ...
  return (
    ...
    >
      <MetricCard
        ...
        subtitle={`${numberFmt(totalHours, 1)} Std gesamt`}
      />
      <MetricCard
        title="Ø Schichten/Tag"
        value={numberFmt(avgShiftsPerDay, 1)}
        subtitle="Durchschnittliche Abdeckung"
      />
      <MetricCard
        title="Mitarbeiter-Auslastung"
        value={`${numberFmt(utilization, 1)} %`}
        subtitle={`${uniqueScheduledEmployees}/${totalEmployees} im Einsatz`}
      />
      <MetricCard
        title="Pausen hinterlegt"
        value={`${numberFmt(breakCoverage, 1)} %`}
        subtitle="Anteil Schichten mit Pause"
      />
    </div>
  );
}
