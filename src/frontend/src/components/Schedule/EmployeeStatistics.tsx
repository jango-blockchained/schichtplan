...
export const EmployeeStatistics = React.memo(function EmployeeStatistics({
  employeeId,
  schedules,
  contractedHours,
  employeeGroup,
  ...
  const weeklyHoursBreakdown = useMemo(() => {
    ...
    weeks.forEach((weekStart) => {
      ...
      employeeSchedules.forEach((schedule) => {
        if (!schedule.date || !schedule.shift_start || !schedule.shift_end)
          return;
      ...
      breakdown.push({
        ...
        percentage:
          effectiveContractedHours > 0
            ? (weekHours / effectiveContractedHours) * 100
            : 0,
      });
    });
  ...
  const monthlyHoursCumulative = useMemo(() => {
    ...
    months.forEach((monthStart) => {
      ...
      employeeSchedules.forEach((schedule) => {
        ...
        const scheduleDate = parseISO(schedule.date);
        if (
          isWithinInterval(scheduleDate, { start: monthStart, end: monthEnd })
        ) {
    ...
    <div className="bg-card dark:bg-card p-6 rounded-lg border border-border dark:border-border min-h-[300px]">
      <Card className="shadow-none border-none bg-transparent">
        <CardContent className="p-0 space-y-3">
          ...
          {weeklyHoursBreakdown.length > 0 &&
            weeklyHoursBreakdown.length <= 4 && (
              <div>
                <div className="text-sm font-medium text-foreground dark:text-foreground mb-2">
                  Wochenstunden
                </div>
          ...
          {monthlyHoursCumulative.length > 0 &&
            monthlyHoursCumulative.length <= 2 && (
