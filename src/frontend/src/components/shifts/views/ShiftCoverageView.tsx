...
const KeyholderTimeBlock: React.FC<KeyholderTimeBlockProps> = ({
  isBefore,
  widthPercentage,
  minutes,
}) => (
...
export const ShiftCoverageView: React.FC<ShiftCoverageViewProps> = ({
  ...
  const keyholderBeforeMinutes =
    activeSettings.general.keyholder_before_minutes ??
    DEFAULT_KEYHOLDER_MINUTES;
  const keyholderAfterMinutes =
    activeSettings.general.keyholder_after_minutes ?? DEFAULT_KEYHOLDER_MINUTES;
  ...
  const keyholderBeforeWidth = useMemo(() => {
    ...
    const totalMinutes =
      (rangeEndTime.getTime() - rangeStartTime.getTime()) / 60000;
  ...
  const enhancedShifts: EnhancedShift[] = useMemo(() => {
    if (mode === "shifts") {
      return activeShifts.map((shift) => {
        ...
        const isEarlyShift =
          shift.start_time === activeSettings.general.store_opening;
        const isLateShift =
          shift.end_time === activeSettings.general.store_closing;
        ...
        <div className="relative w-full">
          ...
          <div className="space-y-8">
            {DAYS.map((day, dayIndex) => {
              const isStoreOpen =
                activeSettings.general.opening_days[dayIndex.toString()];

              // Get shifts for this day
              const dayShifts = enhancedShifts.filter(
                (shift) =>
                  typeof shift.active_days === "object" &&
                  shift.active_days[dayIndex.toString()],
              );
