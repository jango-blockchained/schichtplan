...
export const ShiftEditModal = ({
    ...
    useEffect(() => {
        if (shiftData.shift_id && shifts) {
            const selectedShift = shifts.find(shift => shift.id === shiftData.shift_id);
            if (selectedShift) {
                setShiftData(prev => ({
                    ...prev,
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            toast({
                title: "Erfolg",
                description: schedule ? "Schicht wurde aktualisiert" : "Schicht wurde erstellt",
            });
            ...
            toast({
                title: "Fehler",
                description: "Die Schicht konnte nicht gespeichert werden. Bitte versuchen Sie es erneut.",
                variant: "destructive"
            });
        } finally {
            setIsLoading(false);
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-[525px]">
                <DialogHeader>
                    <DialogTitle>{schedule ? 'Schicht bearbeiten' : 'Neue Schicht'}</DialogTitle>
                    <DialogDescription>
                        {schedule ? 'Bearbeiten Sie die Schichtdetails unten.' : 'Erstellen Sie eine neue Schicht für den ausgewählten Tag.'}
                    </DialogDescription>
                </DialogHeader>
                        ...
                        <div className="grid gap-4">
                            ...
                            <div className="grid grid-cols-4 items-center gap-4">
                                ...
                                <select
                                    ...
                                    onChange={(e) => setShiftData(prev => ({ ...prev, status: e.target.value }))}
                                >
