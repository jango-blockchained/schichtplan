...
import { AlertCircle, Circle, CheckCircle, XCircle, RefreshCw, Clock, Loader2, Info, AlertTriangle } from 'lucide-react';
...
export const GenerationOverlay: React.FC<GenerationOverlayProps> = ({
    ...
    const isComplete = generationSteps.every(step => step.status === 'completed');
    const hasError = generationSteps.some(step => step.status === 'error');

    if (!showGenerationOverlay) return null;

    return (
        <Dialog open={true} onOpenChange={(open) => !open && resetGenerationState()}>
            <DialogContent className="sm:max-w-lg">
                <DialogHeader>
                    <DialogTitle className="flex items-center gap-2">
                        ...
                        {isComplete ? "Generierung abgeschlossen" : hasError ? "Fehler bei der Generierung" : "Generiere Dienstplan"}
                    </DialogTitle>
                </DialogHeader>
                            ...
                            <ul className="space-y-2">
                                {generationSteps.map((step) => (
                                    <li key={step.id} className="flex items-center gap-3">
                                        ...
                                        {step.message && (
                                            <span className="text-xs text-muted-foreground ml-auto truncate"> - {step.message}</span>
                                        )}
                                    </li>
                                ))}
                            </ul>
                        ...
                        <CardContent>
                            <ScrollArea className="h-[200px] w-full rounded-md border p-3 text-sm">
                                {generationLogs.length === 0 ? (
                                    <p className="text-muted-foreground italic">Keine Logs verfügbar.</p>
                                ) : (
