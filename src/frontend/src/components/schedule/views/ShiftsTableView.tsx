  ...
  if (schedules.length === 0) {
    return (
      <Alert>
        ...
        <AlertDescription>
          Keine Schichten im ausgewählten Zeitraum gefunden
        </AlertDescription>
      </Alert>
    );
  }
        ...
        {selectedDate && !isOpeningDay(selectedDate) && (
          <div className="px-6 pt-1 pb-2">
            <Alert>
              <Info className="h-4 w-4" />
              <AlertDescription>
                Der ausgewählte Tag ist kein regulärer Öffnungstag laut
                Filialeinstellungen.
              </AlertDescription>
            </Alert>
          </div>
        ...
        <CardContent>
          {selectedDateSchedules.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              Keine Schichten für den{" "}
              {selectedDate && format(selectedDate, "dd.MM.yyyy")} geplant
            </div>
          ) : (
            <div className="space-y-6">
              {Object.entries(schedulesByShift).map(
                ([shiftId, shiftSchedules]) => {
