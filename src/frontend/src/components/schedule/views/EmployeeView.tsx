...
export const EmployeeView = ({
  ...
  const employeeSchedules = schedules.filter(
    (schedule) => schedule.employee_id === selectedEmployeeId,
  );
  ...
  const getAbsenceForDay = (day: Date): any | null => {
    if (
      !employeeAbsences ||
      !selectedEmployeeId ||
      !employeeAbsences[selectedEmployeeId]
    ) {
      return null;
    }
  ...
  const getStatistics = () => {
    ...
    employeeSchedules.forEach((schedule) => {
      if (schedule.shift_type_id) {
        shiftTypeCounts[schedule.shift_type_id] =
          (shiftTypeCounts[schedule.shift_type_id] || 0) + 1;
      }
    });
            ...
            <>
              <div className="flex flex-col md:flex-row gap-6 mb-6">
                <div className="flex-1">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-x-4 text-sm">
                      ...
                      <span>
                        {selectedEmployee.is_keyholder ? "Ja" : "Nein"}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {statistics && (
                  <div className="flex-1">
                    <h3 className="text-lg font-medium mb-2">
                      Statistik für ausgewählten Zeitraum
                    </h3>
                    <div className="space-y-2">
                      <div className="grid grid-cols-2 gap-x-4 text-sm">
                        <span className="text-muted-foreground">
                      ...
                      {Object.entries(statistics.shiftTypeCounts).map(
                        ([shiftType, count]) => (
                          <div
                            key={shiftType}
                ...
                <TableBody>
                  {days.map((day) => {
                    ...
                    if (daySchedule?.shift_start && daySchedule?.shift_end) {
                      const [startHour, startMinute] = daySchedule.shift_start
                        .split(":")
                        .map(Number);
                      const [endHour, endMinute] = daySchedule.shift_end
                        .split(":")
                        .map(Number);
                      hours =
                        endHour - startHour + (endMinute - startMinute) / 60;
                    ...
                    return (
                      <TableRow
                        key={dayStr}
                        className={cn(daySchedule ? "" : "opacity-70")}
                      >
                        ...
                        <TableCell>
                          ...
                          ) : daySchedule?.shift_start &&
                            daySchedule?.shift_end ? (
