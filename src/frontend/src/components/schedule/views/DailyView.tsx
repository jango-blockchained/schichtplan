...
export const DailyView = ({
  ...
  const isWithinSchedule = (timeSlot: Date, schedule: Schedule) => {
    ...
    const [startHour, startMinute] = schedule.shift_start
      .split(":")
      .map(Number);
    const [endHour, endMinute] = schedule.shift_end.split(":").map(Number);

    const scheduleStartTime = setMinutes(
      setHours(new Date(), startHour),
      startMinute,
    );
    const scheduleEndTime = setMinutes(
      setHours(new Date(), endHour),
      endMinute,
    );
        ...
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <div className="flex items-center space-x-2">
            <Button
              ...
              disabled={
                !selectedDay ||
                (dateRange.from && selectedDay <= dateRange.from)
              }
            >
            ...
            <Button
              ...
              onClick={goToNextDay}
              disabled={
                !selectedDay || (dateRange.to && selectedDay >= dateRange.to)
              }
            >
            ...
            <>
              ...
              <div className="grid grid-cols-[auto_1fr] gap-4">
                ...
                <div className="space-y-4 pr-4 border-r">
                  ...
                  {/* Spacer to align with employee header */}
                ...
                <div className="overflow-x-auto">
                  <div className="grid grid-cols-[repeat(auto-fill,minmax(150px,1fr))] gap-4">
                    ...
                    {employees?.map((employee) => (
                      <div
                        key={employee.id}
                        className="h-12 flex flex-col justify-center px-2 border-b"
                      >
                    ...
                    {employees?.map((employee) => {
                      ...
                      const absence = selectedDay
                        ? getEmployeeAbsence(employee.id, selectedDay)
                        : null;
                                ...
                                <Tooltip>
                                  ...
                                  <TooltipContent>
                                    <p>
                                      ...
                                      {absence.reason || "Kein Grund angegeben"}
                                    </p>
                                    <p className="text-xs">
                                      {format(
                          ...
                          {!absence &&
                            employeeSchedules.map((schedule) => {
                              ...
                              const [startHour, startMinute] =
                                schedule.shift_start.split(":").map(Number);
                              const [endHour, endMinute] = schedule.shift_end
                                .split(":")
                                .map(Number);
