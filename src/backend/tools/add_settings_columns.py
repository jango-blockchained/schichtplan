    ...
    with app.app_context():
        try:
            ...
            if 'enable_diagnostics' not in existing_columns:
                conn.execute(sa.text('ALTER TABLE settings ADD COLUMN enable_diagnostics BOOLEAN NOT NULL DEFAULT 0'))
                conn.commit()
            ...
            if 'generation_requirements' not in existing_columns:
                conn.execute(sa.text('ALTER TABLE settings ADD COLUMN generation_requirements JSON'))
                conn.commit()
