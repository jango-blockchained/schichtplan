...
def create_test_shifts(conn, cursor):
    ...
    try:
        ...
        shifts = [
            # Early shifts
            {"name": "Morning A", "start_time": "09:00", "end_time": "14:00", "shift_type": "EARLY", "active_days": "0,1,2,3,4,5,6"},
            {"name": "Morning B", "start_time": "10:00", "end_time": "15:00", "shift_type": "EARLY", "active_days": "0,1,2,3,4,5,6"},
            
            # Middle shifts
            {"name": "Midday A", "start_time": "11:00", "end_time": "16:00", "shift_type": "MIDDLE", "active_days": "0,1,2,3,4,5,6"},
            {"name": "Midday B", "start_time": "12:00", "end_time": "17:00", "shift_type": "MIDDLE", "active_days": "0,1,2,3,4,5,6"},
            
            # Late shifts
            {"name": "Evening A", "start_time": "15:00", "end_time": "20:00", "shift_type": "LATE", "active_days": "0,1,2,3,4,5,6"},
            {"name": "Evening B", "start_time": "16:00", "end_time": "21:00", "shift_type": "LATE", "active_days": "0,1,2,3,4,5,6"},
        ]
...
def create_test_coverage(conn, cursor):
    ...
    try:
        ...
        for day in range(7):  # 0=Monday, 6=Sunday in Python's convention
            ...
            blocks = [
                {"start_time": "09:00", "end_time": "13:00", "min_employees": 1, "max_employees": 2},
                {"start_time": "13:00", "end_time": "17:00", "min_employees": 1, "max_employees": 2},
                {"start_time": "17:00", "end_time": "21:00", "min_employees": 1, "max_employees": 2}
            ]
...
def verify_resources_for_scheduler(cursor, test_date):
        ...
        if active_days:
            try:
                ...
                try:
                    ...
                    days_list = [int(day.strip()) for day in active_days.split(',') if day.strip()]
                
                if weekday in days_list:
                    applicable_shifts.append(shift_id)
                    print(f"  ✅ Shift {shift_id} (Type: {shift_type}) is applicable for weekday {weekday}")
                else:
                    print(f"  ❌ Shift {shift_id} (Type: {shift_type}) is NOT applicable for weekday {weekday}")
    ...
    cursor.execute("SELECT id, day_index, start_time, end_time FROM coverage WHERE day_index = ?", (day_index,))
    coverage = cursor.fetchall()
    
    print(f"Coverage records for weekday {weekday} (day_index {day_index}): {len(coverage)}")
    for cov_id, day_index, start_time, end_time in coverage:
        print(f"  Coverage {cov_id}: day_index={day_index}, time {start_time}-{end_time}")
...
def main():
    ...
    try:
        ...
        missing_tables = [t for t in essential_tables if t not in [table[0] for table in tables]]
