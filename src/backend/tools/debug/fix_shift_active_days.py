...
def main():
    ...
    with app.app_context():
        ...
        for shift in shifts:
            if not hasattr(shift, 'active_days') or shift.active_days is None or len(shift.active_days) == 0:
                shifts_missing_active_days.append(shift)
            elif not isinstance(shift.active_days, list) or not all(isinstance(day, int) and 0 <= day <= 6 for day in shift.active_days):
                shifts_with_invalid_active_days.append(shift)
            else:
                print(f"Shift #{shift.id}: active on {shift.active_days} ({', '.join(day_names[day] for day in shift.active_days if 0 <= day <= 6)})")
        ...
        days_with_coverage = [day for day, coverage in coverage_by_day.items() if coverage]
        ...
        if shifts_missing_active_days:
            ...
            for shift in shifts_missing_active_days:
                print(f"  Shift #{shift.id}: {shift.start_time}-{shift.end_time}, {getattr(shift, 'shift_type', 'Unknown type')}")
        ...
        if shifts_missing_active_days or shifts_with_invalid_active_days:
            ...
            if fix_option.lower() == 'y':
                ...
                default_days = days_with_coverage if days_with_coverage else list(range(7))
                
                print(f"Default active days will be set to: {default_days} ({', '.join(day_names[day] for day in default_days)})")
                ...
                if fixed_count > 0:
                    ...
                    for i in range(7):
                        ...
                        active_shifts = [s for s in shifts if hasattr(s, 'active_days') and s.active_days and weekday in s.active_days]
                        print(f"  {test_date} ({day_names[weekday]}): {len(active_shifts)} active shifts")
                else:
        ...
        for day_idx, day_coverage in coverage_by_day.items():
            ...
            active_shifts = [s for s in shifts if hasattr(s, 'active_days') and s.active_days and day_idx in s.active_days]
            
            # Check if there are any active shifts for this day
            if not active_shifts:
                misalignments.append(f"Day {day_idx} ({day_names[day_idx]}) has coverage but no active shifts")
                continue
