    ...
    with app.app_context():
        ...
        for shift in ShiftTemplate.query.all():
            ...
            if isinstance(active_days, dict):
                ...
                active_days_names = [days_map.get(day, f"Unknown({day})") for day in active_days]
                print(f"  Active days: {', '.join(active_days_names)}")
            else:
        ...
        print(f"  Morning (before 10:00): {len(shifts_by_time['morning'])} shifts - {shifts_by_time['morning']}")
        print(f"  Midday (10:00-13:59): {len(shifts_by_time['midday'])} shifts - {shifts_by_time['midday']}")
        print(f"  Evening (14:00+): {len(shifts_by_time['evening'])} shifts - {shifts_by_time['evening']}")
        
        if inconsistent_shifts:
            print("\nInconsistent Type Assignments (type doesn't match time):")
            for shift in inconsistent_shifts:
                print(f"  Shift ID {shift['id']}: Type is {shift['configured_type']} but time {shift['hours']} suggests {shift['inferred_type']}")
        ...
        test_shifts = resources.shifts[:4] if len(resources.shifts) >= 4 else resources.shifts
        ...
        for shift in resources.shifts:
            if hasattr(distribution_manager, '_categorize_shift'):
                try:
                    ...
                    print(f"Shift {shift.id} ({shift.shift_type_id}, {shift.start_time}-{shift.end_time}) categorized as: {category}")
                    
                    # Warn about inconsistencies
                    if category != shift.shift_type_id:
                        print(f"⚠️  WARNING: Distribution manager categorizes shift {shift.id} as {category}, " 
        ...
        if hasattr(distribution_manager, 'filter_shifts_by_type'):
            ...
            for shift_type in ["EARLY", "MIDDLE", "LATE"]:
                try:
                    filtered = distribution_manager.filter_shifts_by_type(resources.shifts, shift_type)
                    print(f"Shifts filtered by {shift_type}: {[s.id for s in filtered]}")
                except Exception as e:
...
def main():
    ...
    print("This tool analyzes shift types and configurations to identify distribution issues.")
