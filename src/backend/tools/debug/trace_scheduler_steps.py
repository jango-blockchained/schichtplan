...
def main():
    ...
    print("=" * 80)
    print("This tool provides detailed step-by-step tracing of the schedule generation process.\n")
        ...
        try:
            ...
            with trace_section("Getting available employees"):
                ...
                if not available_employees:
                    ...
                    for i, emp in enumerate(available_employees[:3]):
                        print(f"  Employee {i+1}: ID={emp.id}, Name={getattr(emp, 'name', 'N/A')}")
            ...
            with trace_section("Testing direct assignment"):
                if needed_shift_instances and available_employees:
                    ...
                    print(f"  Shifts grouped by type: {[(t, len(s)) for t, s in shifts_by_type.items()]}")
                    ...
                    for shift_type, type_shifts in shifts_by_type.items():
                        with trace_section(f"Assigning {shift_type} shifts", indent=2):
                            ...
                            assignments = generator.distribution_manager.assign_employees_by_type(
                                test_date, type_shifts, available_employees, shift_type
                            )
                            
                            if assignments:
                                print(f"    ✅ Created {len(assignments)} {shift_type} assignments")
                                # Show first assignment
                                if assignments:
                                    print("    First assignment:")
                                    pretty_print_json(assignments[0], indent=6)
                            else:
                                print(f"    ⚠️ No assignments created for {shift_type} shifts")
                ...
                with trace_section("Testing specific date coverage"):
                    ...
                    for coverage in coverage_records:
                        if hasattr(coverage, 'day_index'):
                            ...
                            print(f"Testing coverage day_index {day_index} against test date weekday {weekday}")
                            if (day_index - 1) % 7 == weekday:  # Assuming day_index uses 0=Sunday format
                                applicable_records.append(coverage)
                    
                    print(f"Applicable coverage records for {test_date}: {len(applicable_records)}")
                    for i, coverage in enumerate(applicable_records[:3]):
                        print(f"Coverage {i+1}: ID={coverage.id}, day_index={coverage.day_index}")
                            ...
                            try:
                                if isinstance(active_days, str):
                                    ...
                                    try:
                                        ...
                                        active_days_list = [int(d.strip()) for d in active_days.split(',') if d.strip()]
                                    
                                    print(f"  Parsed active_days: {active_days_list}")
                                    if weekday in active_days_list:
                                        applicable_shifts.append(shift)
                                        print(f"  ✅ Shift {shift.id} is applicable for weekday {weekday}")
                                    else:
                                        print(f"  ❌ Shift {shift.id} is NOT applicable for weekday {weekday}")
                                ...
                                print(f"  ⚠️ Error parsing active_days for shift {shift.id}: {e}")
                    
                    print(f"Applicable shift templates for {test_date}: {len(applicable_shifts)}")
                ...
                with trace_section("Testing employee availability"):
                    ...
                    if generator.availability_checker:
                        ...
                        for emp in active_employees:
                            if generator.availability_checker.is_employee_on_leave(emp.id, test_date):
            ...
            if not issues:
                print("1. Check the constraint checker to see if it's rejecting all possible assignments")
                print("2. Try temporarily disabling constraint checks in the distribution manager")
                print("3. Add more detailed logging in the assign_employees_by_type method")
            else:
                for i, issue in enumerate(issues):
