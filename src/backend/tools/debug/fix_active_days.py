...
def main():
    ...
    try:
            ...
            if active_days is None:
                ...
                try:
                    ...
                    if not days_list or not isinstance(days_list, list):
                        print(
                            f"Shift #{shift_id}: Empty or invalid active_days: {active_days}"
                        )
                        shifts_with_invalid_active_days.append(shift)
                    else:
                        days_text = ", ".join(
                            day_names[day] for day in days_list if 0 <= day < 7
                        )
                        print(f"Shift #{shift_id}: active on {days_list} ({days_text})")
                        shifts_with_valid_active_days.append(shift)
                except (json.JSONDecodeError, TypeError, ValueError):
                    print(
                        f"Shift #{shift_id}: Invalid active_days format: {active_days}"
                    )
                    shifts_with_invalid_active_days.append(shift)
        ...
        if shifts_missing_active_days or shifts_with_invalid_active_days:
            ...
            print(
                f"FOUND {len(shifts_with_invalid_active_days)} SHIFTS WITH INVALID ACTIVE_DAYS"
            )
            ...
            if fix_option.lower() == "y":
                ...
                print(
                    f"Default active days will be set to: {default_days} ({', '.join(day_names[day] for day in default_days if 0 <= day < 7)})"
                )
                use_default = input(
                ...
                if fixed_count > 0:
                    ...
                    print(
                        f"After update: {active_shift_count} of {len(updated_shifts)} shifts have valid active_days."
                    )
                else:
        ...
        print(
            "If you're still having issues, check that the shift times align with coverage requirements."
        )
