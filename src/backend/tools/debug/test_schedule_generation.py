...
def main():
    ...
    with app.app_context():
        try:
            ...
            empty_entries = [entry for entry in schedule if entry.get('employee_id') is None]
            ...
            if invalid_count > 0:
                ...
                for employee_id, violations in validation_details.items():
                    ...
                    for violation in violations[:2]:  # Show only first 2 violations per employee
                        print(f"    - Date: {violation.get('date')}, Violations: {violation.get('violations')}")
