    ...
    try:
        ...
        with app.app_context():
            ...
            if shifts_needing_update:
                print(
                    f"Found {len(shifts_needing_update)} shifts with missing active_days. Fixing..."
                )
            ...
            if date_shifts:
                print(
                    f"First date shift: Type={date_shifts[0].get('shift_type')}, Hours={date_shifts[0].get('duration_hours')}"
                )
            ...
            if date_shifts:
                ...
                if assignments:
                    ...
                    for i, assignment in enumerate(assignments[:3]):
                        ...
                        print(
                            f"  {i + 1}. Employee {emp_id} assigned to Shift {shift_id}"
                        )
