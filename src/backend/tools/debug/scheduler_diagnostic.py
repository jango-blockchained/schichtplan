...
with app.app_context():
    ...
    try:
        ...
        from models import Employee, ShiftTemplate, Coverage, db, Absence, EmployeeAvailability
    except ImportError:
        try:
            ...
            from backend.models import Employee, ShiftTemplate, Coverage, db, Absence, EmployeeAvailability
        except ImportError:
            try:
                ...
                from src.backend.models import Employee, ShiftTemplate, Coverage, db, Absence, EmployeeAvailability
            except ImportError as e:
...
def diagnose_resources(resources):
    ...
    if len(shifts) == 0:
        ...
        shift_types = {}
        for shift in shifts:
            shift_type = getattr(shift, "shift_type", None) or getattr(shift, "shift_type_id", "Unknown")
        ...
        shifts_with_active_days = [s for s in shifts if hasattr(s, "active_days") and s.active_days]
    ...
    if len(coverage) == 0:
        ...
        for day_idx in range(7):
            ...
            total_employees_needed = sum(getattr(c, "min_employees", 0) for c in day_coverage)
            
            print(f"  • {days[day_idx]}: {len(day_coverage)} records, {total_employees_needed} total employees needed")
...
def diagnose_date_coverage(resources, generator, date_to_check):
    ...
    try:
        ...
        for shift_id, count in shift_needs.items():
            ...
            shift_type = getattr(shift, "shift_type_id", "Unknown") if shift else "Unknown Type"
            print(f"  • Shift {shift_id} ({shift_name}, {shift_type}): {count} employees needed")
...
def diagnose_date_shifts(resources, generator, date_to_check):
    ...
    try:
        ...
        for idx, shift in enumerate(date_shifts, 1):
            ...
            print(f"  {idx}. Shift {shift_id} ({shift_type}): {start_time} - {end_time}")
...
def diagnose_availability(resources, generator, date_to_check, distribution_manager=None):
    ...
    if distribution_manager:
        available_employees = distribution_manager.get_available_employees(date_to_check, date_shifts)
        print(f"✅ Found {len(available_employees)} generally available employees out of {len(active_employees)} active.")
        ...
        if len(available_employees) == 0:
            ...
            for shift_type, type_shifts in shifts_by_type.items():
                available_for_type = distribution_manager.get_available_employees(date_to_check, type_shifts)
                print(f"  • {shift_type}: {len(available_for_type)} employees available")
        ...
        for employee in active_employees:
            is_on_leave = availability_checker.is_employee_on_leave(employee.id, date_to_check)
            ...
            for shift in date_shifts:
                ...
                if shift_template:
                    is_available, avail_type = availability_checker.is_employee_available(
                        employee.id, date_to_check, shift_template
                    )
                    
                    if is_available:
                        available_shifts.append(f"Shift {shift_template.id} ({avail_type})")
                    else:
                        unavailable_shifts.append(f"Shift {shift_template.id}")
            
            if available_shifts:
                available_by_employee[employee.id] = f"✅ Available for: {', '.join(available_shifts)}"
                available_count += 1
            else:
                available_by_employee[employee.id] = f"❌ Not available for any shifts"
        
        print(f"✅ Found {available_count} available employees out of {len(active_employees)} active.")
...
def diagnose_constraints(resources, generator, date_to_check):
    ...
    print(f"  • Max Consecutive Days: {getattr(config, 'max_consecutive_days', 'Not set')}")
    print(f"  • Min Rest Hours: {getattr(config, 'min_rest_hours', 'Not set')}")
    print(f"  • Enforce Rest Periods: {getattr(config, 'enforce_rest_periods', 'Not set')}")
    ...
    if hasattr(config, 'employee_types'):
        ...
        for emp_type in config.employee_types:
            print(f"  • {emp_type.get('id', 'Unknown')}: {emp_type.get('max_daily_hours', 'Not set')} hours")
            ...
            try:
                ...
                if len(assignments) == 0:
                    ...
                    while current_date <= end_date:
                        ...
                        has_coverage = diagnose_date_coverage(resources, generator, current_date)
                        if not has_coverage:
                            print("\n⚠️  No coverage requirements found for this date, skipping further diagnostics.")
                            current_date += timedelta(days=1)
                            continue
                        
                        has_shifts = diagnose_date_shifts(resources, generator, current_date)
                        if not has_shifts:
                            print("\n⚠️  No applicable shifts found for this date, skipping further diagnostics.")
                            current_date += timedelta(days=1)
                            continue
                        
                        has_available = diagnose_availability(resources, generator, current_date)
...
def main():
    ...
    print("This tool runs the schedule generator and diagnoses why shifts might not be assigned.")
