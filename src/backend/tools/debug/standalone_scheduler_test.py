...
def load_shifts_from_db(cursor) -> List[MockShiftTemplate]:
    ...
    shifts = []
    for row in cursor.fetchall():
        shift_id, start_time, end_time, duration_hours, shift_type, active_days, shift_type_id = row
...
def main():
    ...
    try:
        ...
        if employees:
            print(f"First employee: ID={employees[0].id}, Name={getattr(employees[0], 'name', 'N/A')}, Keyholder={employees[0].is_keyholder}")
        ...
        if coverage:
            print(f"First coverage: ID={coverage[0].id}, Day={coverage[0].day_index}, Min={coverage[0].min_employees}")
        ...
        if hasattr(generator, 'constraint_checker'):
            ...
            generator.constraint_checker.config.max_consecutive_days = 10  # Very high to allow assignments
        ...
        try:
            ...
            print(f"  Metric totals: {metrics.get('total_shifts', 0)} shifts distributed")
