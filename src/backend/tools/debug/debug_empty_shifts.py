...
def check_shift_templates(db):
    ...
    for i, shift in enumerate(shifts, 1):
        ...
        print(f"  ID: {shift.id}")
        print(
            f"  Type: {shift.shift_type.value if hasattr(shift, 'shift_type') and shift.shift_type else 'MISSING'}"
        )
        ...
        if hasattr(shift, "active_days") and shift.active_days:
            active_day_names = [
                day_names[day] for day in shift.active_days if 0 <= day < 7
            ]
...
def check_coverage(db):
    ...
    if not coverage:
        print(
            "ERROR: No coverage requirements found. This will prevent schedule generation."
        )
        return False
    ...
    for day_idx in range(7):
        ...
        for i, cov in enumerate(day_coverage, 1):
            ...
            print(
                f"    Requires keyholder: {getattr(cov, 'requires_keyholder', False)}"
            )

            if cov.min_employees == 0:
                print(
                    "    WARNING: min_employees is 0 - no employees will be scheduled"
                )
...
def check_employees(db):
    ...
    if not active_employees:
        print(
            "ERROR: No active employees found. This will prevent schedule generation."
        )
        return False

    if not keyholders:
        print(
            "WARNING: No keyholders found. This may prevent scheduling shifts requiring keyholders."
        )
    ...
    for i, emp in enumerate(active_employees[:5], 1):
        ...
        print(
            f"  Name: {getattr(emp, 'first_name', '')} {getattr(emp, 'last_name', '')}"
        )
...
def check_employee_availability(db, test_date=None):
    ...
    absences = Absence.query.filter(
        Absence.start_date <= test_date, Absence.end_date >= test_date
    ).all()

    print(f"Employees on leave for {test_date}: {len(absences)}")
    for absence in absences:
        print(
            f"  Employee ID {absence.employee_id}: {absence.start_date} to {absence.end_date}"
        )
    ...
    print(
        f"Availability records for day {test_date.weekday()}: {len(availability_records)}"
    )

    # Check each employee's availability
    for employee in employees[:5]:  # Check first 5 for brevity
        employee_records = [
            r for r in availability_records if r.employee_id == employee.id
        ]

        if not employee_records:
            print(
                f"  Employee ID {employee.id}: No explicit availability records (assumed available)"
            )
        else:
            unavailable_hours = [
                f"{r.start_hour}:00-{r.end_hour}:00"
                for r in employee_records
                if hasattr(r, "availability_type")
                and r.availability_type == "UNAVAILABLE"
            ]

            if unavailable_hours:
                print(
                    f"  Employee ID {employee.id}: Unavailable during {', '.join(unavailable_hours)}"
                )
            else:
                print(
                    f"  Employee ID {employee.id}: Has {len(employee_records)} availability records, all available"
                )
...
def check_scheduler_config(db):
    ...
    print(
        f"  Min hours between shifts: {getattr(settings, 'min_hours_between_shifts', 'Not set')}"
    )
    print(
        f"  Max consecutive days: {getattr(settings, 'max_consecutive_days', 'Not set')}"
    )
    ...
    if (
        hasattr(settings, "min_hours_between_shifts")
        and settings.min_hours_between_shifts > 12
    ):
        constraint_issues.append(
            f"min_hours_between_shifts is set to {settings.min_hours_between_shifts} hours, which may be too restrictive"
        )

    if hasattr(settings, "max_consecutive_days") and settings.max_consecutive_days < 3:
        constraint_issues.append(
            f"max_consecutive_days is set to {settings.max_consecutive_days}, which may be too restrictive"
        )
...
def test_schedule_generator(app, start_date=None, end_date=None):
    ...
    if len(entries_with_shifts) == 0:
        ...
        print(
            "  4. Constraint violations prevent assignments (rest periods, consecutive days)"
        )
...
def check_date_shift_matches(app, test_date=None):
    ...
    for shift in all_shifts:
        if (
            hasattr(shift, "active_days")
            and shift.active_days
            and day_index in shift.active_days
        ):
    ...
    for coverage in coverage_blocks:
        print(
            f"  Coverage: {coverage.start_time}-{coverage.end_time}, min={coverage.min_employees}"
        )
    ...
    if active_shifts and coverage_blocks:
        for coverage in coverage_blocks:
            ...
            if not matching_shifts:
                print(
                    f"  WARNING: No shifts match coverage block {coverage.start_time}-{coverage.end_time}"
                )
                has_mismatches = True
    ...
    if not has_mismatches:
        print(
            "  Good news! Shifts and coverage requirements appear to be aligned for this day."
        )
...
def run_diagnostic():
    ...
    with app.app_context():
        ...
        print("=================================================================")
        print(
            f"Shift Templates: {'✅ OK' if shift_templates_ok else '❌ Issues Found'}"
        )
        print(f"Coverage Requirements: {'✅ OK' if coverage_ok else '❌ Issues Found'}")
        print(
            f"Employee Configuration: {'✅ OK' if employees_ok else '❌ Issues Found'}"
        )
        print(f"Scheduler Configuration: {'✅ OK' if config_ok else '❌ Issues Found'}")
        print(
            f"Date-Shift Alignment: {'✅ OK' if date_shift_matches_ok else '❌ Issues Found'}"
        )
        print(
            f"Employee Availability: {'✅ OK' if availability_ok else '❌ Issues Found'}"
        )
        print(
            f"Schedule Generation: {'✅ Shifts Assigned' if generator_ok else '❌ No Shifts Assigned'}"
        )
        ...
        if not all_checks_passed:
            ...
            if not generator_ok:
                ...
                print(
                    "- Check if constraints are too restrictive (rest periods, consecutive days)"
                )
        else:
            print(
