...
def fix_shift_types():
    ...
    with app.app_context():
        ...
        for shift in shifts:
            ...
            print(
                f"  Active days: {shift.active_days} (type: {type(shift.active_days).__name__})"
            )
        ...
        if inconsistent_shifts:
            ...
            for shift in inconsistent_shifts:
                print(
                    f"  Shift ID {shift['id']}: Currently {shift['current_type']}, "
            ...
            for shift_info in inconsistent_shifts:
                ...
                print(
                    f"  Updated Shift ID {shift.id} from {old_type} to {shift.shift_type_id}"
                )
        ...
        if inconsistent_format:
            ...
            for shift in shifts:
                if isinstance(shift.active_days, list):
                    ...
                    print(
                        f"  Updated Shift ID {shift.id} active_days from {old_format} to {shift.active_days}"
                    )
