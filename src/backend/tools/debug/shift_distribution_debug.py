...
def main():
    ...
    print("=" * 80)
    print(
        "This tool inspects the distribution process to find why shifts aren't being assigned.\n"
    )
    ...
    with app.app_context():
        ...
        for shift in shifts:
            ...
            print(
                f"Shift ID: {shift.id}, Type: {shift.shift_type}, Active days: {active_days}"
            )
            if (
                not active_days
                or not isinstance(active_days, (list, set, str))
                or len(str(active_days)) == 0
            ):
        ...
        for shift_type in ["EARLY", "MIDDLE", "LATE"]:
            ...
            if dist_manager.availability_checker:
                ...
                for emp in employees:
                    for shift in type_shifts:
                        is_available, avail_type = (
                            dist_manager.availability_checker.is_employee_available(
                ...
                print(
                    f"Type {shift_type}: {available_for_type} employees available for {len(type_shifts)} shifts"
                )
            else:
                print(
                    f"Type {shift_type}: Cannot check availability (availability_checker not available)"
                )
        ...
        try:
            ...
            if assignments:
                ...
                try:
                    keyholder_assignments = dist_manager.assign_keyholders(
                        test_date, date_shifts
                    )
                    print(
                        f"Keyholder assignments attempted: {len(keyholder_assignments)}"
                    )
                except Exception as e:
                ...
                for shift_type in ["EARLY", "MIDDLE", "LATE"]:
                    type_shifts = [
                        s for s in date_shifts if s.get("shift_type") == shift_type
                    ]
                    if not type_shifts:
                    ...
                    available_employees = dist_manager.get_available_employees(
                        test_date, type_shifts
                    )
                    print(
                        f"Type {shift_type}: {len(available_employees)} employees available for {len(type_shifts)} shifts"
                    )
                    ...
                    if available_employees:
                        try:
                            ...
                            print(
                                f"Type {shift_type} assignments: {len(type_assignments)}"
                            )
                        except Exception as e:
                            print(
                                f"Error assigning employees for type {shift_type}: {str(e)}"
                            )
                    else:
