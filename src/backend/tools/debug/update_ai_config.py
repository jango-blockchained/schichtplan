...
def update_ai_config():
    ...
    try:
        ...
        if not db_path:
            print("Database not found in common locations. Please specify the path manually.")
            return
        ...
        if row:
            ...
            # Update the settings
            cursor.execute("UPDATE settings SET ai_scheduling = ? WHERE id = ?", 
            ...
            # Verify the update
            cursor.execute("SELECT ai_scheduling FROM settings WHERE id = ?", (settings_id,))
            ...
            # Insert new settings
            cursor.execute("INSERT INTO settings (ai_scheduling) VALUES (?)", (ai_config_json,))
            conn.commit()
