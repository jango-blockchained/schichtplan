...
class MockResources:
    ...
    def __init__(self):
        ...
        self.employees = [
            ...
            self._create_employee(
                4, "Alice", "Williams", True, False, ["EARLY", "MIDDLE"]
            ),
            self._create_employee(
                5, "Charlie", "Brown", True, True, ["MIDDLE", "LATE"]
            ),
        ]
    ...
    def _create_employee(
        self, id, first_name, last_name, is_active, is_keyholder, preferred_shifts
    ):
...
class MockAvailabilityChecker:
    ...
    def is_employee_available(self, employee_id, date_to_check, shift_template):
        ...
        shift_type = getattr(shift_template, "shift_type_id", None) or getattr(
            shift_template, "shift_type", None
        )
    ...
    try:
        ...
        if assignments:
            ...
            for idx, assignment in enumerate(assignments, 1):
                ...
                logger.info(
                    f"  {idx}. Employee {employee_id} assigned to shift {shift_id} ({shift_type})"
                )
            ...
            for shift in shifts:
                ...
                if shift_id not in assigned_shift_ids:
                    logger.warning(
                        f"Shift {shift_id} ({shift.get('shift_type')}) was not assigned"
                    )
        else:
...
def run_multi_day_test():
    ...
    for test_date in test_dates:
        ...
        try:
            ...
            logger.info(
                f"Day {test_date} - {len(assignments)}/{len(shifts)} shifts assigned"
            )
    ...
    for emp_id, count in sorted(employee_totals.items()):
        ...
        multiple_shifts_days = [
            date for date, count in employee_by_day[emp_id].items() if count > 1
        ]
        if multiple_shifts_days:
            logger.warning(
                f"  Employee {emp_id} has multiple shifts on: {multiple_shifts_days}"
            )
    ...
    if employee_totals:
        ...
        logger.info(
            f"Distribution stats - Min: {min_shifts}, Max: {max_shifts}, Avg: {avg_shifts:.2f}"
        )
        ...
        if max_shifts - min_shifts > 3:
            logger.warning(
                "Potentially unfair distribution detected - high variance between employees"
            )
...
if __name__ == "__main__":
    try:
        ...
        if not single_day_assignments:
            logger.error(
                "SINGLE-DAY TEST FAILED: Distribution algorithm did not generate any assignments"
            )
            sys.exit(1)
        else:
            logger.info(
                f"SINGLE-DAY TEST PASSED: Distribution algorithm generated {len(single_day_assignments)} assignments"
            )
        ...
        if not multi_day_assignments:
            logger.error(
                "MULTI-DAY TEST FAILED: Distribution algorithm did not generate any assignments"
            )
            sys.exit(1)
        else:
            logger.info(
                f"MULTI-DAY TEST PASSED: Distribution algorithm generated {len(multi_day_assignments)} assignments"
            )
