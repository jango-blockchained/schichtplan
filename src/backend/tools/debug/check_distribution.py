...
def check_coverage_shift_distribution():
    ...
    with app.app_context():
        ...
        print(
            f"Testing distribution for date: {test_date} (Weekday: {test_date.weekday()})"
        )
        ...
        for shift in shifts:
            if (
                ...
                and test_date.weekday() in shift.active_days
            ):
                print(
                    f"Shift #{shift.id}: {shift.start_time}-{shift.end_time} (active)"
                )
                active_shifts.append(shift)
            else:
                print(
                    f"Shift #{shift.id}: {shift.start_time}-{shift.end_time} (inactive - not scheduled for this day)"
                )
        ...
        for coverage in coverages:
            if coverage.day_index == test_date.weekday():
                print(
                    f"Coverage: day={coverage.day_index}, time={coverage.start_time}-{coverage.end_time}, min={coverage.min_employees}"
                )
        ...
        for coverage in applicable_coverage:
            ...
            for shift in active_shifts:
                ...
                shift_start_hour, shift_start_min = map(
                ...
                if shift_start_mins < cov_end_mins and shift_end_mins > cov_start_mins:
                    overlap_percent = min(shift_end_mins, cov_end_mins) - max(
                        shift_start_mins, cov_start_mins
                    )
                    overlap_percent = (
                        overlap_percent / (cov_end_mins - cov_start_mins) * 100
                    )
                    suitable_shifts.append((shift, overlap_percent))
                    print(
                        f"  Shift #{shift.id} ({shift.start_time}-{shift.end_time}) covers {overlap_percent:.1f}% of this block"
                    )
        ...
        for coverage in applicable_coverage:
            ...
            if not matched:
                print(
                    f"Coverage block {coverage.start_time}-{coverage.end_time} has no exact matching shift"
                )
        ...
        if exact_matches:
            print(
                f"Found {len(exact_matches)} exact matches between coverage blocks and shifts"
            )
            for coverage, shift in exact_matches:
                print(
                    f"  Coverage {coverage.start_time}-{coverage.end_time} matches Shift #{shift.id}"
                )
        ...
        if not exact_matches:
            print(
                "The problem is likely that NO shifts match coverage blocks EXACTLY in time."
            )
            print("In the current implementation, the generator does the following:")
            print("1. For each day, it creates all active shifts")
            print(
                "2. It then filters these to keep only shifts that EXACTLY match coverage times"
            )
            print(
                "3. If no shifts match the coverage times exactly, no assignments will be made"
            )

            print("\nPOSSIBLE SOLUTIONS:")
            print(
                "1. Modify your shift templates to match coverage block times exactly"
            )
            print(
                "2. Update the scheduler code to consider overlapping shifts, not just exact matches"
            )
            print(
                "   - Look for the '_generate_assignments_for_date' method in ScheduleGenerator"
            )
        else:
            # Must be another issue
            print("Shifts and coverage blocks do align correctly. The issue may be:")
            print(
                "1. Employee availability - check if employees are available during these times"
            )
            print(
                "2. Constraints preventing assignment - check scheduler logs for constraint violations"
            )
        ...
        if not exact_matches:
            ...
            for shift in active_shifts:
                print(
                    f"   - Add coverage block for day {test_date.weekday()} from {shift.start_time} to {shift.end_time}"
                )

            print("\n2. OR update your shift templates to match your coverage blocks:")
            for coverage in applicable_coverage:
                print(
                    f"   - Add/modify shift template with times from {coverage.start_time} to {coverage.end_time}"
                )
