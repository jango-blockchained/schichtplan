...
def main():
    ...
    try:
        ...
        if employees:
            print(f"First employee: ID={employees[0].id}, Name={employees[0].name}, Keyholder={employees[0].is_keyholder}")
        
        shifts = load_shifts_from_db(cursor)
        print(f"Loaded {len(shifts)} shift templates")
        if shifts:
            print(f"First shift: ID={shifts[0].id}, Type={shifts[0].shift_type}, Hours={shifts[0].duration_hours}")
        
        coverage = load_coverage_from_db(cursor)
        print(f"Loaded {len(coverage)} coverage records")
        if coverage:
            print(f"First coverage: ID={coverage[0].id}, Day={coverage[0].day_index}, Min staffing={coverage[0].min_employees}")
        ...
        if date_shifts:
            ...
            if assignments:
                ...
                for i, assignment in enumerate(assignments[:3]):
                    print(f"  {i+1}. Employee {assignment.get('employee_id')} assigned to Shift {assignment.get('shift_id')}")
