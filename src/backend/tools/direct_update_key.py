...
def update_gemini_api_key_directly():
    ...
    if not DB_PATH.exists():
        ...
        print("Please make sure the application has been run at least once to initialize the database.")
        return
    ...
    try:
        ...
        if result:
            ...
            try:
                ...
                print("Warning: Existing ai_scheduling value is not valid JSON. Resetting.")
                ai_settings = {}
        ...
        if result:
            cursor.execute("UPDATE settings SET value=? WHERE key=?", (ai_settings_json, "ai_scheduling"))
        else:
            cursor.execute("INSERT INTO settings (key, value) VALUES (?, ?)", ("ai_scheduling", ai_settings_json))
