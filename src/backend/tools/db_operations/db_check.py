...
with app.app_context():
    ...
    for version in versions:
        ...
        print(
            f"  With shift assignments: {with_shifts} ({(with_shifts / version_count * 100) if version_count else 0:.1f}%)"
        )
        ...
        for i, schedule in enumerate(sample):
            print(
                f"    {i + 1}. Employee: {schedule.employee_id}, Date: {schedule.date}, Shift: {schedule.shift_id}"
            )
