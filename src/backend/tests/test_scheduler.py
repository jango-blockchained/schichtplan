#!/usr/bin/env python
"""
Script to test the schedule generation process with improved demo data.
...
@click.command("test-scheduler")
@with_appcontext
def test_scheduler_command():
    """Test the schedule generation process with improved demo data."""
    ...
    try:
        ...
        for i, warning in enumerate(warnings[:5]):
            ...
            logger.info(f"  {i + 1}. {warning.get('message')} ({warning.get('type')})")
...
@click.command("test-scheduler-dates")
...
@click.option("--days", default=7, help="Number of days if no dates provided")
@with_appcontext
def test_scheduler_with_dates_command(start_date, end_date, days):
    """Test the schedule generation process with specific dates."""
    logger.info("Starting schedule generation test with specified dates...")
    ...
    try:
        ...
        # Load existing data or use demo data
        existing_employees = Employee.query.count()
        if existing_employees == 0:
            logger.info("No employees found, generating demo data...")
            employees = generate_improved_employee_data()
            generate_improved_coverage_data()
            generate_improved_shift_templates()
            generate_improved_availability_data(employees)
        else:
            logger.info(f"Using existing data: {existing_employees} employees found")

        # Clear any existing schedules in the date range
        logger.info("Clearing existing schedules in date range...")
...
@click.command("test-scheduler-employees")
@click.option("--tl", default=3, help="Number of team leaders")
@click.option("--vz", default=7, help="Number of full-time employees")
@click.option("--tz", default=12, help="Number of part-time employees")
@click.option("--gfb", default=8, help="Number of mini-job employees")
@click.option("--days", default=7, help="Number of days to schedule")
@click.option("--reset", is_flag=True, help="Reset all data before testing")
@with_appcontext
def test_scheduler_with_employee_counts(tl, vz, tz, gfb, days, reset):
    """Test the schedule generation with different employee counts."""
    logger.info(
        f"Starting schedule generation with custom employee counts: TL={tl}, VZ={vz}, TZ={tz}, GFB={gfb}"
    )
    ...
    try:
        ...
        # Generate data with custom employee counts
        ...
        logger.info("Generating employee data with custom distribution...")
        # Create a custom version of generate_improved_employee_data
        employees = generate_employees_with_custom_counts(employee_types)

        logger.info("Generating availability data...")
        availabilities = generate_improved_availability_data(employees)

        # Clear schedules in the date range
        logger.info("Clearing existing schedule data...")
        Schedule.query.filter(
            Schedule.date >= start_date, Schedule.date <= end_date
        ).delete()
...
def generate_employees_with_custom_counts(employee_types):
    """Generate employee data with custom distribution"""
    settings = Settings.query.first()
    if not settings:
        settings = Settings.get_default_settings()
        db.session.add(settings)
...
# Register the command to the Flask application
def register_commands(app):
    app.cli.add_command(test_scheduler_command)
    app.cli.add_command(test_scheduler_with_dates_command)
    app.cli.add_command(test_scheduler_with_employee_counts)
