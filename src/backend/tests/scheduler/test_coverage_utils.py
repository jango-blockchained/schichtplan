...
class MockCoverage:
    def __init__(
        ...
        self.employee_types = (
            employee_types if employee_types is not None else []
        )  # List of strings
...
class TestGetRequiredStaffingForInterval(unittest.TestCase):
    def setUp(self):
        ...
        type(self.mock_resources.config).interval_duration_minutes = PropertyMock(
            return_value=60
        )

    def test_simple_coverage_match(self):
        # Monday, 09:00-17:00, 2 employees, Type A
        coverage1 = MockCoverage(
            id=1,
            day_index=0,
            start_time_str="09:00",
            end_time_str="17:00",
            min_employees=2,
            employee_types=["TypeA"],
        )
        ...
        needs = get_required_staffing_for_interval(
            test_date, interval_start, self.mock_resources
        )
    ...
    def test_overlapping_coverage_max_employees_and_combine_types_keyholder(self):
        ...
        coverage2 = MockCoverage(
            id=2,
            day_index=0,
            start_time_str="10:00",
            end_time_str="13:00",
            min_employees=3,
            employee_types=["TypeB"],
            requires_keyholder=True,
        )
        # Monday, 09:30-10:30, 1 employee, Type C
        coverage3 = MockCoverage(
            id=3,
            day_index=0,
            start_time_str="09:30",
            end_time_str="10:30",
            min_employees=1,
            employee_types=["TypeC"],
        )
        ...
        needs = get_required_staffing_for_interval(
            test_date, interval_start, self.mock_resources, interval_duration_minutes=60
        )

        self.assertEqual(
            needs["min_employees"],
            3,
            "Should take max of min_employees from overlapping coverages",
        )
        ...
        self.assertTrue(
            needs["requires_keyholder"],
            "Should be True if any overlapping coverage requires keyholder",
        )
    ...
    def test_partial_overlap_start_of_coverage(self):
        ...
        self.assertEqual(
            needs["min_employees"],
            0,
            "Interval starts before coverage, should not match",
        )

        # Interval 10:00-11:00. This interval IS covered.
        interval_start_covered = time(10, 0)
        needs_covered = get_required_staffing_for_interval(
            test_date,
            interval_start_covered,
            self.mock_resources,
            interval_duration_minutes=60,
        )
        self.assertEqual(
            needs_covered["min_employees"],
            1,
            "Interval starts at coverage start, should match",
        )
    ...
    def test_partial_overlap_end_of_coverage(self):
        ...
        self.assertEqual(
            needs["min_employees"],
            1,
            "Interval ends after coverage start but before/at coverage end, should match",
        )
        ...
        needs_not_covered = get_required_staffing_for_interval(
            test_date,
            interval_start_not_covered,
            self.mock_resources,
            interval_duration_minutes=60,
        )
        self.assertEqual(
            needs_not_covered["min_employees"],
            0,
            "Interval starts at coverage end, should not match",
        )
    ...
    def test_coverage_just_outside_boundaries(self):
        ...
        needs_before = get_required_staffing_for_interval(
            test_date,
            interval_start_before,
            self.mock_resources,
            interval_duration_minutes=60,
        )
        ...
        needs_after = get_required_staffing_for_interval(
            test_date,
            interval_start_after,
            self.mock_resources,
            interval_duration_minutes=60,
        )
    ...
    def test_time_str_to_datetime_time(self):
        ...
        with self.assertRaises(
            AttributeError
        ):  # Or TypeError depending on handling of None
