...
DEFAULT_SETTINGS_DICT_EXPECTATION = {
    ...
    "pdf_layout": {
        ...
        "table_style": {
            "header_bg_color": "#E5E7EB", "border_color": "#D1D5DB",
            "text_color": "#1F2937", "header_text_color": "#111827",
        },
        "fonts": {"family": "Arial", "size": 9.0, "header_size": 11.0},
        "content": {
            "show_employee_id": False, "show_position": True,
            "show_breaks": True, "show_total_hours": True,
        },
    },
    "employee_groups": {
        "employee_types": [
    ...
    "availability_types": {
        "types": [
            {"id": "AVAILABLE", "name": "Verfügbar", "description": "Mitarbeiter ist verfügbar", "color": "#22c55e", "priority": 2, "is_available": True, "type": "availability_type"},
            {"id": "FIXED", "name": "Fix", "description": "Feste Arbeitszeiten", "color": "#3b82f6", "priority": 1, "is_available": True, "type": "availability_type"},
            {"id": "PREFERRED", "name": "Bevorzugt", "description": "Bevorzugte Arbeitszeiten", "color": "#f59e0b", "priority": 3, "is_available": True, "type": "availability_type"},
            {"id": "UNAVAILABLE", "name": "Nicht verfügbar", "description": "Mitarbeiter ist nicht verfügbar", "color": "#ef4444", "priority": 4, "is_available": False, "type": "availability_type"},
        ]
    },
...
def test_get_default_settings_to_dict(app: Flask):
    ...
    with app.app_context():
        ...
        if 'availability_types' in settings_dict and 'types' in settings_dict['availability_types']:
        ...
        for category, expected_values in DEFAULT_SETTINGS_DICT_EXPECTATION.items():
            assert category in settings_dict, f"Category {category} missing in settings_dict"
            # Deep comparison for nested dicts, careful with list order if not guaranteed
            if isinstance(expected_values, dict):
                for key, expected_value in expected_values.items():
                    assert key in settings_dict[category], f"Key {key} missing in settings_dict[{category}]"
                    if isinstance(expected_value, list):
                         # Sort lists of dicts by 'id' if present, for stable comparison
                        if all(isinstance(item, dict) and 'id' in item for item in expected_value):
                            expected_sorted = sorted(expected_value, key=lambda x: x['id'])
                            actual_sorted = sorted(settings_dict[category][key], key=lambda x: x['id'])
                            assert actual_sorted == expected_sorted, f"Mismatch in {category}.{key}"
                        else:
                            assert settings_dict[category][key] == expected_value, f"Mismatch in {category}.{key}"
                    else:
                        assert settings_dict[category][key] == expected_value, f"Mismatch in {category}.{key}"
            else:
...
def test_update_settings_from_dict(app: Flask):
    ...
    with app.app_context():
        ...
        update_data = {
            ...
            "employee_groups": {
                "employee_types": [ # Full replacement
                    {"id": "NEW_VZ", "name": "New Vollzeit", "min_hours": 30, "max_hours": 38, "type": "employee_type"}
                ],
                "absence_types": [] # Clear all
            },
            "availability_types": { # Full replacement
                "types": [
                    {"id": "WORK", "name": "Working", "color": "#00FF00", "priority": 1, "is_available": True, "type": "availability_type"}
                ]
            },
            "actions": {
                "demo_data": {"selected_module": "employees", "last_execution": "2025-01-01T10:00:00"}
            },
            "ai_scheduling": {
                "enabled": True, "api_key": "test_key_123"
            }
        }
        ...
        assert updated_settings_dict["general"]["opening_days"]["tuesday"] == True # Check if others preserved
        ...
        assert updated_settings_dict["scheduling"]["generation_requirements"]["enforce_max_hours"] == False
        assert updated_settings_dict["scheduling"]["generation_requirements"]["enforce_minimum_coverage"] == True # Check preserved
        ...
        assert updated_settings_dict["employee_groups"]["employee_types"][0]["id"] == "NEW_VZ"
        ...
        # Actions assertions
        assert updated_settings_dict["actions"]["demo_data"]["selected_module"] == "employees"
...
def test_get_settings_api(client):
    ...
    assert data["general"]["store_name"] == DEFAULT_SETTINGS_DICT_EXPECTATION["general"]["store_name"]
    assert len(data["employee_groups"]["employee_types"]) == len(DEFAULT_SETTINGS_DICT_EXPECTATION["employee_groups"]["employee_types"])
...
def test_update_settings_api(client):
    ...
    update_payload = {
        ...
        "employee_groups": {
            "shift_types": [{"id": "NIGHT", "name": "Nacht", "color": "#000000", "type": "shift_type", "auto_assign_only": False}]
        }
    }
...
def test_update_settings_api_partial_employee_groups(client):
    ...
    initial_employee_types_count = len(initial_settings_response.json["employee_groups"]["employee_types"])
    initial_absence_types_count = len(initial_settings_response.json["employee_groups"]["absence_types"])

    update_payload = {
        "employee_groups": {
            "shift_types": [{"id": "SPECIAL_SHIFT", "name": "Sonderschicht", "color": "#FF00FF", "type": "shift_type", "auto_assign_only": False}]
    ...
    assert len(data["employee_groups"]["employee_types"]) == initial_employee_types_count
    ...
    assert len(get_data["employee_groups"]["employee_types"]) == initial_employee_types_count
    assert len(get_data["employee_groups"]["absence_types"]) == initial_absence_types_count
