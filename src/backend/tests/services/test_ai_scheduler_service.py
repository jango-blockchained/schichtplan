...
try:
    from src.backend.services.ai_scheduler_service import AISchedulerService as AIService
...
class TestAISchedulerService(unittest.TestCase):
    ...
    def test_load_api_key_from_settings_success(self, mock_logger):
        ...
        mock_logger.app_logger.info.assert_any_call("GEMINI_API_KEY loaded successfully.")
    ...
    @patch('src.backend.services.ai_scheduler_service.logger')
    def test_collect_data_for_ai_prompt_success(self, mock_logger, MockAbsence, MockEmployeeAvailability, MockCoverage, MockShiftTemplate, MockEmployee):
        ...
        mock_class_date_attr.__eq__ = MagicMock(return_value=mock_comparison_expression) # Just in case
        ...
        MockEmployeeAvailability.query.filter.return_value.all.return_value = [mock_avail]
        ...
        self.assertIn(json.dumps([{"id": 1, "name": "John Doe", "role": "Cashier", "is_keyholder": False}], indent=2), result_text)
        
        self.assertIn("Shift Templates:", result_text)
        self.assertIn(json.dumps([{"id": 10, "name": "Morning", "start_time": "08:00:00", "end_time": "16:00:00", "active_days": [0,1,2,3,4]}], indent=2), result_text)

        self.assertIn("Coverage Needs:", result_text)
        self.assertIn(json.dumps([{"id": 100, "shift_template_id": 10, "min_employees": 1, "max_employees": 2, "days_of_week": [0,1,2,3,4]}], indent=2), result_text)

        self.assertIn(f"Employee Availability ({start_date_obj} to {end_date_obj}):", result_text)
        ...
        expected_avail_json_part = {"employee_id": 1, "availability_type": "Available", "raw_availability_record": expected_avail_record}
        self.assertIn(json.dumps([expected_avail_json_part], indent=2), result_text)


        self.assertIn(f"Employee Absences ({start_date_obj} to {end_date_obj}):", result_text)
        self.assertIn(json.dumps([{"employee_id": 1, "start_date": "2024-01-02", "end_date": "2024-01-02", "reason": "Sick"}], indent=2), result_text)

        self.assertIn("General Scheduling Rules:", result_text)
        self.assertIn(json.dumps({ "max_consecutive_shifts": 5, "min_rest_between_shifts_hours": 11, "max_weekly_hours": 40 }, indent=2), result_text)
        
        mock_logger.app_logger.info.assert_any_call(f"Collecting data for AI prompt from {start_date_obj} to {end_date_obj}.")
        mock_logger.app_logger.info.assert_any_call("Data collection for AI prompt complete.")
    ...
    @patch('src.backend.services.ai_scheduler_service.ShiftTemplate')
    @patch('src.backend.services.ai_scheduler_service.Employee')
    def test_collect_data_for_ai_prompt_db_errors(self, MockEmployee, MockShiftTemplate, MockCoverage, MockEmployeeAvailability, MockAbsence, mock_logger):
        """Test data collection when various database queries fail."""
        # Setup side effects for model queries to simulate DB errors
        MockEmployee.query.filter_by.return_value.all.side_effect = Exception("DB Employee Error")
        ...
        MockEmployeeAvailability.query.filter.side_effect = Exception("DB Availability Error")
        ...
        self.assertIn("Employee Availability/Absences: Error collecting data.", result_text) # Combined error message

        # Verify errors were logged
        mock_logger.app_logger.error.assert_any_call("Error collecting employee data: DB Employee Error", exc_info=True)
        mock_logger.app_logger.error.assert_any_call("Error collecting shift template data: DB ShiftTemplate Error", exc_info=True)
        mock_logger.app_logger.error.assert_any_call("Error collecting coverage data: DB Coverage Error", exc_info=True)
        ...
        mock_logger.app_logger.error.assert_any_call("Error collecting availability/absence data: DB Availability Error", exc_info=True)
    ...
    def test_construct_system_prompt(self, mock_logger):
        ...
        collected_data_text = "Employees:\n[]\n\nShift Templates:\n[]\n\nCoverage Needs:\n[]"

        prompt = service._construct_system_prompt(collected_data_text, start_date_obj, end_date_obj)

        self.assertIn(f"Schedule Period: {start_date_obj.isoformat()} to {end_date_obj.isoformat()}", prompt)
        self.assertIn("Output Format:", prompt)
        self.assertIn("Please provide the schedule STRICTLY in CSV format.", prompt)
        self.assertIn("EmployeeID,Date,ShiftTemplateID,ShiftName,StartTime,EndTime", prompt)
        self.assertIn("Provided Data:", prompt)
        self.assertIn(collected_data_text, prompt)
        self.assertIn("Please generate the schedule assignments in CSV format now.", prompt)
        
        mock_logger.app_logger.info.assert_any_call("Constructing system prompt for AI.")
        ...
        if mock_logger.app_logger.isEnabledFor(logging.DEBUG):
            mock_logger.app_logger.debug.assert_called_with(f"Generated system prompt: {prompt}")
        else:
            mock_logger.app_logger.info.assert_called_with(f"Generated system prompt (first 500 chars): {prompt[:500]}...")
    ...
    def test_call_gemini_api_success(self, mock_logger, mock_post):
        ...
        mock_response.json.return_value = {
            "candidates": [{
                "content": {"parts": [{"text": expected_csv_text}]}
            }]
        }
        ...
        mock_logger.app_logger.info.assert_any_call(f"Calling Gemini API model: {service.gemini_model_name}")
        mock_logger.app_logger.info.assert_any_call("Successfully received response from Gemini API.")
        mock_logger.app_logger.info.assert_any_call(f"Extracted CSV text from Gemini API response (length: {len(expected_csv_text.strip())}).")
    ...
    def test_call_gemini_api_http_error(self, mock_logger, mock_post):
        ...
        mock_response.json.side_effect = json.JSONDecodeError("msg", "doc", 0) # Simulate no valid JSON in error
    ...
    def test_call_gemini_api_request_exception(self, mock_logger, mock_post):
        ...
        self.assertIn("Gemini API request failed: Connection timed out", str(context.exception))
        mock_logger.app_logger.error.assert_any_call("Request error calling Gemini API: Connection timed out", exc_info=True)
    ...
    def test_call_gemini_api_invalid_response_structure(self, mock_logger, mock_post):
        ...
        mock_response = MagicMock()
        mock_response.json.return_value = {"error": "Some error"} # Missing 'candidates'
        ...
        self.assertIn("Invalid response structure from Gemini API", str(context.exception))
        mock_logger.app_logger.error.assert_any_call(
    ...
    def test_call_gemini_api_empty_csv_text(self, mock_logger, mock_post):
        ...
        mock_logger.app_logger.warning.assert_any_call("Gemini API returned an empty CSV string.")
    ...
    def test_call_gemini_api_no_api_key(self, mock_logger):
        ...
        mock_logger.app_logger.error.assert_any_call("Gemini API key is not configured. Cannot make API call.")
    ...
    def test_parse_csv_response_valid_data(self, mock_logger):
        ...
        assignments = service._parse_csv_response(csv_text, start_date_obj, end_date_obj)
        ...
        self.assertEqual(assignments[0]["raw_row"], ["1","2024-08-01","10","Morning","08:00","16:00"])

        self.assertEqual(assignments[1]["employee_id"], 2)
        self.assertEqual(assignments[1]["date"], date(2024, 8, 2))
        mock_logger.app_logger.info.assert_any_call(f"Parsing CSV response (length: {len(csv_text)}).")
        mock_logger.app_logger.info.assert_any_call("CSV header validated successfully.")
        mock_logger.app_logger.info.assert_any_call(f"Successfully parsed {len(assignments)} valid assignments from CSV.")

    @patch('src.backend.services.ai_scheduler_service.logger')
    def test_parse_csv_response_empty_input(self, mock_logger):
        """Test parsing an empty CSV string."""
        service = AIService() # type: ignore
        assignments = service._parse_csv_response("", date(2024,1,1), date(2024,1,1))
        self.assertEqual(assignments, [])
        mock_logger.app_logger.warning.assert_any_call("CSV response is empty or contains no header.")
    ...
    def test_parse_csv_response_no_header(self, mock_logger):
        ...
        assignments = service._parse_csv_response("\n", date(2024,1,1), date(2024,1,1)) # Only a newline
        self.assertEqual(assignments, [])
        mock_logger.app_logger.warning.assert_any_call("CSV response is empty or contains no header.")
    ...
    def test_parse_csv_response_mismatched_header(self, mock_logger):
        ...
        mock_logger.app_logger.error.assert_any_call(f"CSV header mismatch. Expected: {expected_header}, Got: {normalized_actual_header}")
    ...
    def test_parse_csv_response_incorrect_column_count(self, mock_logger):
        ...
        assignments = service._parse_csv_response(csv_text, date(2024,8,1), date(2024,8,7))
        self.assertEqual(len(assignments), 0) # Both rows should be skipped
        mock_logger.app_logger.warning.assert_any_call("Row 2: Incorrect number of columns. Expected 6, got 5. Row: ['1', '2024-08-01', '10', 'Morning', '08:00']")
        mock_logger.app_logger.warning.assert_any_call("Row 3: Incorrect number of columns. Expected 6, got 7. Row: ['2', '2024-08-02', '11', 'Evening', '16:00', '23:00', 'ExtraCol']")
        mock_logger.app_logger.warning.assert_any_call("Finished parsing CSV. Total malformed/skipped rows: 2")
    ...
    def test_parse_csv_response_invalid_data_types(self, mock_logger):
        ...
        mock_logger.app_logger.warning.assert_any_call("Row 2: Error converting data type: invalid literal for int() with base 10: 'abc'. Row: ['abc', '2024-08-01', '10', 'Morning', '08:00', '16:00']")
        mock_logger.app_logger.warning.assert_any_call("Row 3: Invalid date format 'bad-date'. Row: ['1', 'bad-date', '10', 'Morning', '08:00', '16:00']")
        mock_logger.app_logger.warning.assert_any_call("Row 4: Error converting data type: invalid literal for int() with base 10: 'xyz'. Row: ['2', '2024-08-02', 'xyz', 'Evening', '16:00', '23:00']")
        mock_logger.app_logger.warning.assert_any_call("Row 5: Invalid time format for 'bad-time' or '23:00'. Row: ['3', '2024-08-03', '11', 'Late', 'bad-time', '23:00']")
        mock_logger.app_logger.warning.assert_any_call("Row 6: Invalid time format for '23:00' or 'bad-time'. Row: ['4', '2024-08-04', '12', 'Night', '23:00', 'bad-time']")
        mock_logger.app_logger.warning.assert_any_call("Finished parsing CSV. Total malformed/skipped rows: 5")
    ...
    def test_parse_csv_response_invalid_ids_and_date_range(self, mock_logger):
        ...
        assignments = service._parse_csv_response(csv_text, start_date, end_date)
        self.assertEqual(len(assignments), 0)
        mock_logger.app_logger.warning.assert_any_call("Row 2: Invalid EmployeeID 0. Must be > 0. Row: ['0', '2024-08-01', '10', 'Morning', '08:00', '16:00']")
        mock_logger.app_logger.warning.assert_any_call("Row 3: Invalid ShiftTemplateID 0. Must be > 0. Row: ['1', '2024-08-01', '0', 'Morning', '08:00', '16:00']")
        mock_logger.app_logger.warning.assert_any_call("Row 4: Assignment date 2024-07-31 out of range. Row: ['2', '2024-07-31', '10', 'PastShift', '08:00', '16:00']")
        mock_logger.app_logger.warning.assert_any_call("Row 5: Assignment date 2024-08-08 out of range. Row: ['3', '2024-08-08', '11', 'FutureShift', '16:00', '23:00']")
        mock_logger.app_logger.warning.assert_any_call("Row 6: ShiftName is empty. Row: ['4', '2024-08-01', '12', '', '08:00', '16:00']")
        mock_logger.app_logger.warning.assert_any_call("Finished parsing CSV. Total malformed/skipped rows: 5")
    ...
    def test_parse_csv_response_mixed_valid_invalid(self, mock_logger):
        ...
        mock_logger.app_logger.warning.assert_any_call("Row 3: Error converting data type: invalid literal for int() with base 10: 'bad'. Row: ['bad', '2024-08-01', '10', 'InvalidEmp', '08:00', '16:00']")
        mock_logger.app_logger.warning.assert_any_call("Row 5: Incorrect number of columns. Expected 6, got 5. Row: ['3', '2024-08-03', '12', 'MissingTime', '16:00']")
        mock_logger.app_logger.warning.assert_any_call("Finished parsing CSV. Total malformed/skipped rows: 2")
        mock_logger.app_logger.info.assert_any_call(f"Successfully parsed {len(assignments)} valid assignments from CSV.")
    ...
    @patch('src.backend.services.ai_scheduler_service.ShiftTemplate')
    @patch('src.backend.services.ai_scheduler_service.logger')
    def test_store_assignments_success(self, mock_logger, MockShiftTemplate, MockEmployee, MockSchedule, mock_db):
        ...
        mock_class_version_attr.__eq__ = MagicMock(return_value=mock_comparison_expression)
        MockSchedule.version = mock_class_version_attr

        parsed_assignments = [
            {"employee_id": 1, "date": date(2024,8,1), "shift_template_id": 10, "shift_name_from_ai": "Morning", "start_time": time(8,0), "end_time": time(16,0), "raw_row": []},
            {"employee_id": 2, "date": date(2024,8,1), "shift_template_id": 11, "shift_name_from_ai": "Evening", "start_time": time(16,0), "end_time": time(23,0), "raw_row": []}
        ]
        version_id = "v1-ai"
        start_date_obj = date(2024,8,1)
        end_date_obj = date(2024,8,7)
        ...
        MockEmployee.query.get.side_effect = lambda eid: mock_emp1 if eid == 1 else (mock_emp2 if eid == 2 else None)

        # Mock ShiftTemplate.query.get
        mock_st10 = MagicMock(id=10)
        mock_st11 = MagicMock(id=11)
        MockShiftTemplate.query.get.side_effect = lambda sid: mock_st10 if sid == 10 else (mock_st11 if sid == 11 else None)
        ...
        stored_count = service._store_assignments(parsed_assignments, version_id, start_date_obj, end_date_obj)
        ...
        self.assertIsInstance(first_add_call_args, MockSchedule) # It will be an instance of the *mocked* Schedule
        ...
        mock_logger.app_logger.info.assert_any_call(f"Storing {len(parsed_assignments)} assignments for version {version_id} from {start_date_obj} to {end_date_obj}.")
        mock_logger.app_logger.info.assert_any_call(f"Deleted existing schedules for version {version_id} in date range.")
        mock_logger.app_logger.info.assert_any_call(f"Successfully added {stored_count} new assignments to DB.")
    ...
    @patch('src.backend.services.ai_scheduler_service.ShiftTemplate')
    @patch('src.backend.services.ai_scheduler_service.logger')
    def test_store_assignments_no_parsed_data(self, mock_logger, MockShiftTemplate, MockEmployee, MockSchedule, mock_db):
        service = AIService() # type: ignore
        stored_count = service._store_assignments([], "v1", date(2024,1,1), date(2024,1,1))
    ...
    @patch('src.backend.services.ai_scheduler_service.ShiftTemplate')
    @patch('src.backend.services.ai_scheduler_service.logger')
    def test_store_assignments_employee_not_found(self, mock_logger, MockShiftTemplate, MockEmployee, MockSchedule, mock_db):
        service = AIService() # type: ignore
        ...
        parsed_assignments = [
            {"employee_id": 999, "date": date(2024,8,1), "shift_template_id": 10, "shift_name_from_ai": "GhostShift", "raw_row": ["999","2024-08-01","10","GhostShift","08:00","16:00"]}
        ]
        MockEmployee.query.get.return_value = None # Employee 999 not found
        MockShiftTemplate.query.get.return_value = MagicMock(id=10) # ShiftTemplate 10 exists

        stored_count = service._store_assignments(parsed_assignments, "v1", date(2024,8,1), date(2024,8,7))
        self.assertEqual(stored_count, 0)
        mock_logger.app_logger.warning.assert_any_call("Employee with ID 999 not found. Skipping assignment for row: ['999', '2024-08-01', '10', 'GhostShift', '08:00', '16:00']")
        mock_db.session.add.assert_not_called()
        ...
        mock_logger.app_logger.info.assert_any_call("No valid new schedule entries to add after processing deletions.")
    ...
    @patch('src.backend.services.ai_scheduler_service.ShiftTemplate')
    @patch('src.backend.services.ai_scheduler_service.logger')
    def test_store_assignments_shifttemplate_not_found(self, mock_logger, MockShiftTemplate, MockEmployee, MockSchedule, mock_db):
        service = AIService() # type: ignore
        ...
        parsed_assignments = [
            {"employee_id": 1, "date": date(2024,8,1), "shift_template_id": 999, "shift_name_from_ai": "PhantomShift", "raw_row": ["1","2024-08-01","999","PhantomShift","08:00","16:00"]}
        ]
        ...
        stored_count = service._store_assignments(parsed_assignments, "v1", date(2024,8,1), date(2024,8,7))
        self.assertEqual(stored_count, 0)
        mock_logger.app_logger.warning.assert_any_call("ShiftTemplate with ID 999 not found. Skipping assignment for row: ['1', '2024-08-01', '999', 'PhantomShift', '08:00', '16:00']")
        mock_db.session.add.assert_not_called()
        mock_logger.app_logger.info.assert_any_call("No valid new schedule entries to add after processing deletions.")
        mock_db.session.commit.assert_called_once()
    ...
    def test_store_assignments_delete_fails(self, mock_logger, MockSchedule, mock_db):
        ...
        parsed_assignments = [{"employee_id": 1, "date": date(2024,8,1), "shift_template_id": 10, "shift_name_from_ai": "Morning"}]
        MockSchedule.query.filter.return_value.delete.side_effect = SQLAlchemyError("Delete failed")

        with self.assertRaisesRegex(RuntimeError, "Failed to clear existing schedule data."):
            service._store_assignments(parsed_assignments, "v1", date(2024,8,1), date(2024,8,7))
        
        mock_db.session.rollback.assert_called_once()
        mock_logger.app_logger.error.assert_any_call("Error deleting existing schedule assignments: Delete failed", exc_info=True)

    @patch('src.backend.services.ai_scheduler_service.db')
    @patch('src.backend.services.ai_scheduler_service.Schedule') # Needed for delete part
    @patch('src.backend.services.ai_scheduler_service.Employee')
    @patch('src.backend.services.ai_scheduler_service.ShiftTemplate')
    @patch('src.backend.services.ai_scheduler_service.logger')
    def test_store_assignments_add_fails(self, mock_logger, MockShiftTemplate, MockEmployee, MockSchedule, mock_db):
        service = AIService() # type: ignore
        ...
        parsed_assignments = [
            {"employee_id": 1, "date": date(2024,8,1), "shift_template_id": 10, "shift_name_from_ai": "Morning", "raw_row": []}
        ]
        MockEmployee.query.get.return_value = MagicMock(id=1)
        ...
        with self.assertRaisesRegex(RuntimeError, "Failed to save new assignments to database."):
            service._store_assignments(parsed_assignments, "v1", date(2024,8,1), date(2024,8,7))
        
        self.assertEqual(mock_db.session.rollback.call_count, 1) # Should be 1 due to the add failure. The SUT has nested try-except for commit.
        mock_logger.app_logger.error.assert_any_call("Database error storing new assignments: Commit failed", exc_info=True)
