...
from src.backend.models import db, Employee # Import db and Employee for potential future use in tests
from flask import current_app # Import current_app here


def test_resource_loading_within_app_context(app, session): # Accept app and session fixtures
    ...
    with app.app_context(): # Use the app fixture's context
        ...
        print(f"Debug: db instance ID in test within app context: {id(db)}") # Debug print
        
        # Test a simple query using the session fixture
        try:
            employee_count = session.query(Employee).count()
            print(f"Debug: Successfully queried employee count using session fixture: {employee_count}")
        ...
        try:
            ...
            print("Resource loading test passed: Resources loaded successfully within app context.")
        except Exception as e:
