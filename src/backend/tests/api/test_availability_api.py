...
def test_update_availability_valid(client, new_availability):
    ...
    response = client.put(
        f'/api/availability/{new_availability.id}',
        json=update_data
    )
    ...
    # Verify in database
    updated_availability = db.session.get(
        EmployeeAvailability, new_availability.id
    )
    assert updated_availability is not None
    assert (
        updated_availability.availability_type.value ==
...
def test_get_shifts_for_employee_valid(client, new_employee):
    ...
    valid_date = '2024-11-01'
    response = client.get(
        '/api/availability/shifts_for_employee?date={}&employee_id={}'
        .format(valid_date, new_employee.id)
    )
    assert response.status_code == HTTPStatus.OK
    data = response.get_json()
    assert isinstance(data, list)
...
def test_get_shifts_for_employee_invalid_date(client, new_employee):
    ...
    response = client.get(
        '/api/availability/shifts_for_employee?date={}&employee_id={}'
        .format(invalid_date, new_employee.id)
    )
    assert response.status_code == HTTPStatus.BAD_REQUEST
    data = response.get_json()
    assert 'Invalid input' in data['message']
...
def test_get_shifts_for_employee_invalid_employee_id(client):
    ...
    response = client.get(
        '/api/availability/shifts_for_employee?date={}&employee_id={}'
        .format(valid_date, invalid_employee_id)
    )
    assert response.status_code == HTTPStatus.BAD_REQUEST
    data = response.get_json()
    assert 'Invalid input' in data['message']
...
def test_get_shifts_for_employee_missing_params(client):
    ...
    # Missing date
    response = client.get(
        '/api/availability/shifts_for_employee?employee_id=1'
    )
    assert response.status_code == HTTPStatus.BAD_REQUEST
    data = response.get_json()
    assert 'Invalid input' in data['message']
    ...
    # Missing employee_id
    response = client.get(
        '/api/availability/shifts_for_employee?date=2024-11-01'
    )
    assert response.status_code == HTTPStatus.BAD_REQUEST
    data = response.get_json()
    assert 'Invalid input' in data['message']
