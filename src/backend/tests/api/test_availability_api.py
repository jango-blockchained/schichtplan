...
def test_create_availability_valid(client, new_employee):
    ...
    availability = EmployeeAvailability.query.filter_by(
        employee_id=new_employee.id, day_of_week=0, hour=9
    ).first()
...
def test_create_availability_invalid_data(client, new_employee):
    """
    Test creating an availability record with invalid data.
    (e.g., missing fields)
    """
    availability_data = {
        "employee_id": new_employee.id,
...
def test_update_availability_valid(client, new_availability):
    """Test updating an existing availability record with valid data."""
    update_data = {"is_available": False, "availability_type": "FIXED"}
    ...
    assert updated_availability.availability_type.value == AvailabilityType.FIXED.value
...
def test_check_availability_available(client, new_employee):
    """Test checking availability for an employee who is available."""
    # Assume new_availability marks employee as available for
...
def test_check_availability_invalid_data(client, new_employee):
    ...
    check_data = {"employee_id": new_employee.id, "date": "not a date", "hour": 10}
    response = client.post("/api/availability/check", json=check_data)
    assert response.status_code == HTTPStatus.BAD_REQUEST
    data = response.get_json()
    assert "Invalid input" in data["message"]
...
def test_update_employee_availabilities_valid(client, new_employee):
    ...
    response = client.put(
        f"/api/availability/employees/{new_employee.id}/availabilities",
        json={"availabilities": update_data},
    )
    assert response.status_code == HTTPStatus.OK
    data = response.get_json()
    ...
    # Verify in database
    updated_availabilities = EmployeeAvailability.query.filter_by(
        employee_id=new_employee.id
    ).all()
    assert len(updated_availabilities) == 2

    # Check the updated records - order might not be guaranteed,
    # so check by content
    updated_data_in_db = [
        {
    ...
    update_data_sorted = sorted(
        update_data, key=lambda x: (x["day_of_week"], x["hour"])
    )
...
def test_update_employee_availabilities_invalid_data(client, new_employee):
    ...
    response = client.put(
        f"/api/availability/employees/{new_employee.id}/availabilities",
        json={"availabilities": invalid_data},
    )
    assert response.status_code == HTTPStatus.BAD_REQUEST
    data = response.get_json()
    assert "Invalid input" in data["message"]
...
def test_get_employee_status_by_date_valid(client, new_employee):
    ...
    # Add more assertions here to check the structure/content of the response
...
def test_get_shifts_for_employee_valid(client, new_employee):
    """Test getting shifts for an employee on a date with valid parameters."""
    valid_date = "2024-11-01"
    response = client.get(
        "/api/availability/shifts_for_employee?date={}&employee_id={}".format(
            valid_date, new_employee.id
        )
    )
    assert response.status_code == HTTPStatus.OK
    data = response.get_json()
...
def test_get_shifts_for_employee_invalid_date(client, new_employee):
    """
    Test getting shifts for an employee on a date with invalid date.
    """
    invalid_date = "not-a-date"
    response = client.get(
        "/api/availability/shifts_for_employee?date={}&employee_id={}".format(
            invalid_date, new_employee.id
        )
    )
    assert response.status_code == HTTPStatus.BAD_REQUEST
    data = response.get_json()
...
def test_get_shifts_for_employee_invalid_employee_id(client):
    """
    Test getting shifts for an employee on a date with invalid
    ...
    response = client.get(
        "/api/availability/shifts_for_employee?date={}&employee_id={}".format(
            valid_date, invalid_employee_id
        )
    )
    assert response.status_code == HTTPStatus.BAD_REQUEST
    data = response.get_json()
...
def test_get_shifts_for_employee_missing_params(client):
    ...
    # Missing date
    response = client.get("/api/availability/shifts_for_employee?employee_id=1")
    assert response.status_code == HTTPStatus.BAD_REQUEST
    data = response.get_json()
    assert "Invalid input" in data["message"]
    assert "details" in data
    ...
    # Missing employee_id
    response = client.get("/api/availability/shifts_for_employee?date=2024-11-01")
    assert response.status_code == HTTPStatus.BAD_REQUEST
    data = response.get_json()
    assert "Invalid input" in data["message"]
    assert "details" in data
