...
class TestEmployeeAPI:
    ...
    def test_employee_model(self, app, session):
        ...
        try:
            ...
            assert str(employee.employee_id) == "JDO" # Ensure comparison is with string value
...
class TestAbsenceAPI:
    def test_create_absence_valid(self, app, session):
        ...
        try:
            ...
            new_absence = session.query(Absence).filter_by(employee_id=employee_id, start_date=datetime(2024, 12, 10).date()).first()
    ...
    def test_create_absence_invalid_data(self, app, session):
        ...
        try:
            ...
            # Check for specific error detail (optional, but good practice)
            found_error = any(err.get('loc') == ('start_date', ) for err in data['details'])
            assert found_error is True # Changed to assert True
    ...
    def test_update_absence_invalid_data(self, app, session):
        ...
        try:
            ...
            # Check for specific error detail (optional, but good practice)
            found_error = any(err.get('loc') == ('end_date', ) for err in data['details'])
            assert found_error is True # Changed to assert True
            
            print("✓ PUT /api/absences/<id> endpoint handles invalid data")
        except Exception as e:
            print(f"✗ PUT /api/absences/<id> endpoint invalid data test failed: {str(e)}")
            raise 