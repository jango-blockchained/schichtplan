...
class EmployeeCreateRequest(BaseModel):
    ...
    contracted_hours: Optional[float] = Field(
        0, description="Contracted hours per week."
    )
    is_keyholder: Optional[bool] = Field(
    ...
    phone: Optional[str] = Field(
        None, description="Employee's phone number (optional)."
    )
...
class EmployeeUpdateRequest(BaseModel):
    ...
    employee_group: Optional[str] = Field(
