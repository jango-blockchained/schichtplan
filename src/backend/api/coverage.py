...
def create_coverage():
    ...
    try:
        ...
        # Emit WebSocket event for coverage creation
        emit_event(
            "coverage_updated",
            {
...
def update_coverage(coverage_id):
    ...
    try:
        ...
        # Emit WebSocket event for coverage update
        emit_event(
            "coverage_updated",
            {
...
def delete_coverage(coverage_id):
    ...
    coverage = Coverage.query.get_or_404(coverage_id)
    day_index = coverage.day_index  # Save for emit event

    try:
        db.session.delete(coverage)
        db.session.commit()

        # Emit WebSocket event for coverage deletion
        emit_event(
            "coverage_updated",
            {"coverage_id": coverage_id, "day_index": day_index, "action": "delete"},
        )
...
def bulk_update_coverage():
    ...
    try:
        ...
        # Emit WebSocket event for bulk coverage update
        emit_event(
            "coverage_updated",
            {
                "action": "bulk_update",
                "days_updated": [day["dayIndex"] for day in data],
            },
        )
