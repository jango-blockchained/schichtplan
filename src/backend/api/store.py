...
def get_config():
    ...
    return jsonify({
        ...
        'min_employees_per_shift': config.min_employees_per_shift,
        'max_employees_per_shift': config.max_employees_per_shift,
...
def update_config():
    ...
    try:
        ...
        if 'opening_time' in data:
            config.opening_time = datetime.strptime(data['opening_time'], '%H:%M').time()
        if 'closing_time' in data:
            config.closing_time = datetime.strptime(data['closing_time'], '%H:%M').time()
...
def reset_config():
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({
            'error': 'Could not reset store configuration',
            'details': str(e)
