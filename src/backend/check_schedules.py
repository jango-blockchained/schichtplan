...
with app.app_context():
    ...
    for schedule in version_1_schedules:
        status_value = schedule.status.value if hasattr(schedule, 'status') and schedule.status else 'None'
        print(f"Schedule ID: {schedule.id}, Status: {status_value}, Date: {schedule.date}")
    
    # Check if there are schedules with status PENDING
    try:
        pending_schedules = Schedule.query.filter(Schedule.status == 'PENDING').limit(5).all()
    ...
    version_status_counts = db.session.query(
        Schedule.version, 
        Schedule.status,
        func.count()
    ).group_by(Schedule.version, Schedule.status).all()
