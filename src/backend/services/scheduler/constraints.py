...
class ConstraintChecker:
    ...
    def _get_assignments_for_employee(
        ...
        """
        return [
            asn for asn in assignments_list if asn.get("employee_id") == employee_id
        ]
    ...
    def _parse_assignment_datetime(
        ...
        if not time_str:  # Check if time_str is None or empty
            self.logger.debug(
                f"Time string is None or empty for field {field_name} in assignment {assignment.get('id')}"
            )
            return None
        try:
            ...
            if not isinstance(on_date, date):
                if isinstance(on_date, str):
                    ...
                    self.logger.warning(
                        f"Invalid 'on_date' type ({type(on_date)}) for parsing time string in assignment {assignment.get('id')}"
                    )
                    return None

            h, m = map(int, time_str.split(":"))
            return datetime.combine(
                on_date, datetime.min.time().replace(hour=h, minute=m)
            )
        except ValueError as e:
            self.logger.warning(
                f"Invalid time string '{time_str}' (ValueError: {e}) in assignment for employee {assignment.get('employee_id')} on date {on_date}"
            )
            return None
        except (
            TypeError
        ) as e:  # Catch errors if on_date is not a valid type for combine
            self.logger.warning(
                f"TypeError during time string parsing for '{time_str}' on date {on_date} (Error: {e}) assignment {assignment.get('id')}"
            )
            return None
    ...
    def check_all_constraints(
        ...
        if not employee:
            self.logger.error(
                f"Constraint Check: Employee {employee_id} not found in resources."
            )
            violations.append(
                {
                    "type": "resource_error",
        ...
        new_shift_duration = self._calculate_shift_duration_from_datetimes(
            new_shift_start_dt, new_shift_end_dt
        )
        if new_shift_duration <= 0:
            violations.append(
                {
                    "type": "shift_error",
                    "message": "New shift has invalid or zero duration.",
                }
            )
        ...
        daily_hours_violation = self._check_daily_hours_limit(
            employee,
            new_shift_duration,  # new_shift_duration is calculated at the start of check_all_constraints
        )
    ...
    def _check_max_consecutive_days(
        ...
        employee_assignments = self._get_assignments_for_employee(
            employee.id, existing_assignments
        )
        ...
        for asn in employee_assignments:
            ...
            if isinstance(asn_date_val, date):
                ...
                try:
                    ...
                    self.logger.warning(
                        f"Invalid date string '{asn_date_val}' in existing assignment for employee {employee.id}"
                    )
        ...
        while check_d in all_considered_worked_dates:
            ...
            if (
                current_streak_count > max_consecutive + 5
            ):  # Safety break for very long loops (e.g. misconfigured data)
                self.logger.error(
                    f"Safety break in consecutive day count for employee {employee.id}, date {new_shift_date}"
                )
                break

        if current_streak_count > max_consecutive:
            message = (
                f"Assigning shift on {new_shift_date} would result in {current_streak_count} consecutive workdays "
    ...
    def _check_min_rest_between_shifts(
        ...
        min_rest_hours = getattr(
            self.config, "min_rest_hours", 11.0
        )  # Ensure float for comparison if config value is int
        ...
        for asn in employee_assignments:
            ...
            if asn_date_val is None:
                self.logger.debug(
                    f"Skipping rest check for assignment {asn.get('id')} due to missing date."
                )
                continue
            prev_shift_end = self._parse_assignment_datetime(
        ...
        for asn in employee_assignments:
            ...
            if asn_date_val is None:
                ...
                continue
            next_shift_start = self._parse_assignment_datetime(
                asn, "start_time", asn_date_val
            )

            if next_shift_start and next_shift_start > new_shift_end_dt:
                if (
                    earliest_subsequent_shift_start_dt is None
                    or next_shift_start < earliest_subsequent_shift_start_dt
                ):
                    earliest_subsequent_shift_start_dt = next_shift_start

        if earliest_subsequent_shift_start_dt:
            rest_duration_hours = (
                earliest_subsequent_shift_start_dt - new_shift_end_dt
            ).total_seconds() / 3600.0
            if rest_duration_hours < min_rest_hours:
                message = (
                    f"Insufficient rest after new shift on {new_shift_start_dt.date()}. "
                    f"Required: {min_rest_hours}h, Actual: {rest_duration_hours:.2f}h "
    ...
    def _check_daily_hours_limit(
        ...
        if employee_group_id_attr is not None:
            if hasattr(
                employee_group_id_attr, "value"
            ):  # Handle Enum e.g. EmployeeGroup.VZ.value
        ...
        if not found_group_in_config and employee_group_id_str is not None:
            self.logger.debug(
                f"Employee group '{employee_group_id_str}' for employee {employee.id} not found in config's employee_types. Using default max_daily_hours: {max_daily_hours}h."
            )
        elif employee_group_id_str is None:
            self.logger.debug(
                f"Employee {employee.id} has no specified employee group. Using default max_daily_hours: {max_daily_hours}h."
            )

        if new_shift_duration > max_daily_hours:
            message = (
                f"New shift duration {new_shift_duration:.2f}h exceeds daily limit of {max_daily_hours}h "
    ...
    def _check_weekly_hours_limit(
        ...
        for asn in employee_assignments:
            ...
            if isinstance(asn_date_val, date):
                ...
                try:
                    ...
                    self.logger.warning(
                        f"Invalid date string '{asn_date_val}' in weekly hours check for emp {employee.id}"
                    )
                    continue
            else:
                # If date is not a date object or parseable string, skip this assignment for weekly calculation
                self.logger.debug(
                    f"Skipping assignment with invalid date type ({type(asn_date_val)}) for weekly hours calc for emp {employee.id}"
                )
                continue

            if week_start_date <= asn_date_obj <= week_end_date:
                # The assignment is within the same week as the new shift
                asn_start_dt = self._parse_assignment_datetime(
                    asn, "start_time", asn_date_obj
                )
                asn_end_dt = self._parse_assignment_datetime(
                    asn, "end_time", asn_date_obj
                )
                if asn_start_dt and asn_end_dt:
                    current_weekly_hours += (
                        self._calculate_shift_duration_from_datetimes(
                            asn_start_dt, asn_end_dt
                        )
                    )
                else:
                    self.logger.debug(
                        f"Could not parse start/end time for assignment {asn.get('id')} on {asn_date_obj} for emp {employee.id} during weekly calc."
                    )
        ...
        config_max_weekly_hours = getattr(self.config, "max_hours_per_group", {}).get(
            employee_group_id_str
        )  # Use employee_group_id_str here

        if (
            config_max_weekly_hours is not None
            and projected_weekly_hours > config_max_weekly_hours
        ):
            message = (
                f"Projected weekly hours {projected_weekly_hours:.2f}h (current: {current_weekly_hours:.2f}h + new: {new_shift_duration:.2f}h) "
            ...
            self.logger.debug(f"Employee {employee.id}: {message}")
            return {
                "type": "max_weekly_hours_group",
                "message": message,
                "limit": config_max_weekly_hours,
        ...
        if (
            contracted_hours is not None and contracted_hours > 0
        ):  # Ensure contracted_hours is positive
            contracted_limit_factor = getattr(
                self.config, "contracted_hours_limit_factor", 1.2
            )
            absolute_contracted_limit = contracted_hours * contracted_limit_factor
            if projected_weekly_hours > absolute_contracted_limit:
                message = (
                    f"Projected weekly hours {projected_weekly_hours:.2f}h "
                ...
                self.logger.debug(f"Employee {employee.id}: {message}")
                return {
                    "type": "max_weekly_hours_contract",
                    "message": message,
        ...
        for day_offset in range(7):
            ...
            for entry in entries_on_date:
                if entry.employee_id == employee_id and entry.shift_id is not None:
                    ...
                    if (
                        shift is not None
                    ):  # This 'if' block should be correctly indented
                        duration_hours = 0
                        if (
                            hasattr(shift, "duration_hours")
                            and shift.duration_hours is not None
                        ):
                            duration_hours = shift.duration_hours
                        elif (
                            hasattr(shift, "start_time")
                            and hasattr(shift, "end_time")
                            and shift.start_time
                            and shift.end_time
                        ):
                            duration_hours = self.calculate_shift_duration(
                                shift.start_time, shift.end_time
                            )
                        ...
                        self.logger.warning(
                            f"Shift with ID {entry.shift_id} not found in resources for weekly hours calculation for employee {employee_id}."
                        )
        return weekly_hours
    ...
    def calculate_rest_hours(
        self, prev_shift_end_time_str: str, current_shift_start_time_str: str
    ) -> float:
        """
