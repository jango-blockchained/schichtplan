    ...
    def validate_employee(self, employee: Any) -> tuple[bool, list[str]]:
        ...
        try:
            ...
            if contracted_hours is not None and (
                not isinstance(contracted_hours, (int, float)) or contracted_hours < 0
            ):
        ...
        try:
            ...
            if max_employees is not None:
                ...
                if min_employees is not None and max_employees < min_employees:
                    errors.append(
                        f"max_employees ({max_employees}) cannot be less than min_employees ({min_employees})"
                    )
                ...
                if shift_templates is not None:
                    if not isinstance(shift_templates, list):
                        ...
                        for i, shift_template in enumerate(shift_templates):
                            is_valid, shift_errors = self.validate_shift_template(
                                shift_template
                            )
                            if not is_valid:
