...
class AvailabilityChecker:
    ...
    def is_employee_available_for_date(
        self, employee_id: int, date_to_check: date
    ) -> bool:
        """
        ...
        try:
            # Always use direct database query for day-based availability
            # This ensures we get all records for the day of week regardless of date ranges
            ...
            self.log_debug(
                f"Found {len(availability_records)} availability records for employee {employee_id} on day_of_week {day_of_week}"
            )

            # Check if there are any UNAVAILABLE records for this day
            for record in availability_records:
                if (
                    hasattr(record, "availability_type")
                    and record.availability_type == AvailabilityType.UNAVAILABLE.value
                ):
                    self.log_debug(
                        f"Employee {employee_id} has UNAVAILABLE availability on {date_to_check}"
                    )
                    return False
                elif hasattr(record, "is_available") and record.is_available is False:
                    self.log_debug(
