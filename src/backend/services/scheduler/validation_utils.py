...
def validate_time_range(
    start_time: str, end_time: str, allow_overnight: bool = True
) -> bool:
    """
    ...
    if allow_overnight and end_minutes == start_minutes:
        raise ValidationError(
            f"Start time and end time cannot be the same ({start_time})"
        )
...
def validate_shift_template(shift_template: Any) -> dict[str, Any]:
    ...
    if hasattr(shift_template, "__dict__"):
        ...
        raise ShiftValidationError(
            f"Invalid shift template type: {type(shift_template)}"
        )
    ...
    if data.get("duration_hours") is not None:
        try:
            ...
            errors.append(
                f"duration_hours must be a number, got {type(data['duration_hours'])}"
            )

    if errors:
        raise ShiftValidationError(
            f"Shift template validation failed: {'; '.join(errors)}"
        )
...
def validate_coverage_rule(coverage: Any) -> dict[str, Any]:
    ...
    if data.get("min_employees") is not None:
        try:
            ...
            errors.append(
                f"min_employees must be an integer, got {type(data['min_employees'])}"
            )
    ...
    if data.get("max_employees") is not None:
        try:
            ...
            elif data.get("min_employees") is not None and max_emp < int(
                data["min_employees"]
            ):
                errors.append(
                    f"max_employees ({max_emp}) cannot be less than min_employees ({data['min_employees']})"
                )
        except (ValueError, TypeError):
            errors.append(
                f"max_employees must be an integer, got {type(data['max_employees'])}"
            )
    ...
    if data.get("requires_keyholder") is not None:
        if not isinstance(data["requires_keyholder"], bool):
            try:
                ...
                errors.append(
                    f"requires_keyholder must be boolean, got {type(data['requires_keyholder'])}"
                )

    if errors:
        raise CoverageValidationError(
            f"Coverage validation failed: {'; '.join(errors)}"
        )
...
def validate_employee_data(employee: Any) -> dict[str, Any]:
        ...
        if data.get(bool_field) is not None:
            if not isinstance(data[bool_field], bool):
                try:
                    ...
                    errors.append(
                        f"{bool_field} must be boolean, got {type(data[bool_field])}"
                    )
    ...
    if data.get("contracted_hours") is not None:
        try:
            ...
            errors.append(
                f"contracted_hours must be a number, got {type(data['contracted_hours'])}"
            )
    ...
    if errors:
        raise EmployeeValidationError(
            f"Employee validation failed: {'; '.join(errors)}"
        )
    ...
    for id_field in ["employee_id", "shift_id"]:
        if data.get(id_field) is not None:
            try:
                ...
                errors.append(
                    f"{id_field} must be an integer, got {type(data[id_field])}"
                )
    ...
    for i, item in enumerate(data_list):
        try:
            ...
            results["valid_items"].append(
                {"index": i, "item": item, "result": validation_result}
            )
            ...
            error_info = {"index": i, "item": item, "error": str(e)}
...
def log_validation_results(
    ...
    if "total_items" in results:
        log.info(
            f"Batch validation: {len(results['valid_items'])}/{results['total_items']} items valid "
        ...
        if results["invalid_items"]:
            ...
            if len(results["invalid_items"]) > 5:
                log.error(
                    f"... and {len(results['invalid_items']) - 5} more validation errors"
                )
