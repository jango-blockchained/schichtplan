...
try:
    ...
    from src.backend.models.employee import AvailabilityType, Employee, EmployeeGroup
    ...
    logger.critical(
        f"FATAL ERROR: Could not import backend models required for validator: {e}"
    )
    logger.critical(
        "Please ensure the backend models are accessible in the Python path."
    )
...
try:
    ...
    def get_required_staffing_for_interval(
        resources, target_date, interval_start, interval_end
    ):
...
class ScheduleConfig:
    ...
    max_hours_per_group: dict[EmployeeGroup, int] | None = field(
        ...
        if "EmployeeGroup" in globals() and hasattr(EmployeeGroup, "TZ")
        else {}
    )
    max_shifts_per_group: dict[EmployeeGroup, int] | None = field(
        default_factory=lambda: {
            # Use actual imported EmployeeGroup members
            EmployeeGroup.TZ: 4,
            EmployeeGroup.GFB: 3,
    ...
    def __post_init__(self):
        ...
        if "EmployeeGroup" in globals() and hasattr(
            EmployeeGroup, "TZ"
        ):  # Check if EmployeeGroup is defined
    ...
    def from_settings(cls, settings):
        ...
        config = cls(
            ...
            if hasattr(settings, "min_rest_between_shifts")
            else 11,
            max_hours_per_day=settings.max_daily_hours
            if hasattr(settings, "max_daily_hours")
            else 10,
...
class ScheduleValidator:
    ...
    def validate(
        self,
        schedule_data: list[Schedule | dict[str, Any]],
    ...
    def _validate_coverage(
        self, schedule_data: list[Schedule | dict[str, Any]]
    ) -> None:
        # Even if schedule_data is empty, we still validate coverage against required
        # staffing to surface understaffing for all intervals when applicable.
        ...
        for entry_data in schedule_data:
            ...
            if isinstance(date_val, str):
                try:
                    ...
                    logger.warning(
                        f"Invalid date format in schedule entry: {entry_data}. Skipping."
                    )
                    continue
                ...
                logger.warning(
                    f"Invalid/missing date in schedule entry: {entry_data}. Skipping."
                )
                continue
            # Store the original entry_data along with its parsed date for consistent access later
            valid_schedule_entries.append(
                {"original_entry": entry_data, "parsed_date": entry_date}
            )
        if not valid_schedule_entries:
            # Skip coverage validation for empty schedule
            self.info.append(
                ValidationError(
                    error_type="CoverageValidationSkip",
                    message="Coverage validation skipped for empty schedule",
                    severity="info",
                )
            )
            return
            ...
            min_parsed_date = min(
                item["parsed_date"] for item in valid_schedule_entries
            )
            max_parsed_date = max(
                item["parsed_date"] for item in valid_schedule_entries
            )
        ...
        while current_validation_date <= max_parsed_date:
            # In test mode, precompute an ordered list of intervals
            # to check for the day
            interval_list = None
            if self.test_mode:
                intervals_to_check: set = set()
                for item in valid_schedule_entries:
                    ...
                    start_dt = (
                        _time_str_to_datetime_time(start_str) if start_str else None
                    )
                    end_dt = _time_str_to_datetime_time(end_str) if end_str else None
                    if start_dt and end_dt:
                        # Collect interval starts between start_dt (inclusive)
                ...
                if not intervals_to_check:
                    ...
                    intervals_to_check = {
                        ...
                        for m in range(0, minutes_in_day, self.INTERVAL_MINUTES)
                    }
                interval_list = sorted(intervals_to_check)

            if self.test_mode and interval_list is not None:
                iterator = interval_list
            else:
                # Fallback: iterate whole day by advancing time of day
                iterator = []
                ...
                while current_time_of_day <= day_end_time:
                    iterator.append(current_time_of_day)
                    current_time_of_day = (
                        datetime.combine(date.min, current_time_of_day)
                        + timedelta(minutes=self.INTERVAL_MINUTES)
                    ).time()

            for interval_start_dt_time in iterator:
                try:
                    ...
                    self.errors.append(
                        ValidationError(
                            ...
                            message=(
                                "Failed to retrieve coverage needs for "
                                f"interval {interval_start_dt_time} on "
                                f"{current_validation_date}."
                            ),
                            severity="critical",
                            details={
                ...
                # Safely coerce values that may be MagicMocks in tests
                ...
                required_min_employees = _as_int(
                    interval_needs.get("min_employees", 0), 0
                )
                # Ensure list-like for employee types and coerce to list of str
                raw_types = interval_needs.get("employee_types", [])
                if raw_types is None:
                    required_employee_types = []
                elif isinstance(raw_types, (list, set, tuple)):
                    required_employee_types = [str(t) for t in list(raw_types)]
                else:
                    required_employee_types = [str(raw_types)]

                requires_keyholder_needed = bool(
                    interval_needs.get("requires_keyholder", False)
                )

                # Count actual staffing for this interval from the schedule
                actual_assigned_employees = 0
                actual_keyholders_present = 0
                actual_employee_types_present = defaultdict(
                    int
                )  # Counts of each employee type present
                ...
                for item in valid_schedule_entries:
                    ...
                    if parsed_assignment_date == current_validation_date:
                        assignment_start_time_str: str | None = None
                        assignment_end_time_str: str | None = None
                        employee_id_val: Any | None = None
                        assignment_id_val: Any | None = None  # For logging
                        ...
                        if isinstance(assignment, dict):
                            ...
                            assignment_start_time_str = getattr(
                                assignment, "start_time", None
                            )
                            assignment_end_time_str = getattr(
                                assignment, "end_time", None
                            )
                            employee_id_val = getattr(assignment, "employee_id", None)
                            assignment_id_val = getattr(assignment, "id", None)

                        assignment_start_dt_time: datetime.time | None = (
                            _time_str_to_datetime_time(assignment_start_time_str)
                            if assignment_start_time_str
                            else None
                        )
                        assignment_end_dt_time: datetime.time | None = (
                            _time_str_to_datetime_time(assignment_end_time_str)
                            if assignment_end_time_str
                            else None
                        )

                        if (
                            assignment_start_dt_time is None
                            or assignment_end_dt_time is None
                        ):
                            logger.warning(
                                "Could not parse start/end time for "
                                f"assignment: {assignment}. "
                                "Skipping interval check."
                            )
                            continue

                        if (
                            interval_start_dt_time is not None
                        ):  # Should always be true here
                            if (
                                assignment_start_dt_time
                                <= interval_start_dt_time
                                < assignment_end_dt_time
                            ):
                                actual_assigned_employees += 1
                                if employee_id_val is not None:
                                    employee: ActualEmployee | None = (
                                        None  # For type hinting
                                    )
                                    if TYPE_CHECKING:
                                        employee = self.resources.get_employee(
                                            employee_id_val
                                        )  # Returns ActualEmployee or None
                                    else:
                                        employee = self.resources.get_employee(
                                            employee_id_val
                                        )  # Runtime version
                                    ...
                                    if employee:
                                        # Use key fields from employee
                                        emp_display_id = (
                                            employee.id
                                            if TYPE_CHECKING
                                            and isinstance(employee, ActualEmployee)
                                            else getattr(employee, "id", None)
                                        )
                                        is_keyholder = (
                                        ...
                                        if emp_group is not None:
                                            actual_employee_types_present[
                                                str(emp_group)
                                            ] += 1
                                    else:
                                        logger.warning(
                                            "Could not find employee with ID "
                                            f"{employee_id_val} for assignment "
                                            f"{assignment_id_val}"
                                        )
                ...
                if min_employees_met:
                    ...
                    self.errors.append(
                        ValidationError(
                            ...
                            message=(
                                "Understaffed for interval starting "
                                f"{interval_start_dt_time} on "
                                f"{current_validation_date}. "
                                f"Required: {required_min_employees}, "
                                f"Actual: {actual_assigned_employees}."
                            ),
                            severity="critical",
                            details={
                                "date": str(current_validation_date),
                                ...
                                "actual_assigned_employees": (
                                    actual_assigned_employees
                                ),
                                "interval_needs": (
                                    self._prepare_interval_needs_for_json(
                                        interval_needs
                                    )
                                ),
                                "assigned_employees_in_interval": (
                                    assigned_employee_details_for_interval
                                ),
                            },
                        )
                    ...
                    if actual_keyholders_present > 0:
                        ...
                        self.errors.append(
                            ValidationError(
                                ...
                                message=(
                                    "Missing keyholder for interval starting "
                                    f"{interval_start_dt_time} on "
                                    f"{current_validation_date}."
                                ),
                                severity="critical",
                ...
                # Assumes required_employee_types is a list of strings
                # and actual_employee_types_present is a dict like
                # {'TZ': 1, 'VZ': 0}
                if required_employee_types:
                    unmet_type_needs = []
                    # This part needs refinement based on how employee_types are
                    # specified in interval_needs. Example: if interval_needs
                    # specifies {'min_per_type': {'TZ': 1, 'GFB': 1}}.
                    # For now, assume required_employee_types is a list of types
                    # that must be present at least once.
                    ...
                    if unmet_type_needs:
                        self.errors.append(
                            ValidationError(
                                ...
                                message=(
                                    "Missing required employee type(s) "
                                    f"{', '.join(unmet_type_needs)} for interval "
                                    f"starting {interval_start_dt_time} on "
                                    f"{current_validation_date}."
                                ),
                                ...
                                details={
                                    ...
                                    "actual_types_present_counts": dict(
                                        actual_employee_types_present
                                    ),
                ...
                # No manual time advancement here; the for-loop handles iteration
    ...
    def _prepare_interval_needs_for_json(self, interval_needs_dict: dict) -> dict:
        ...
        if not interval_needs_dict:
            return {}
        processed = interval_needs_dict.copy()
        if "employee_types" in processed and isinstance(
            processed["employee_types"], set
        ):
            processed["employee_types"] = sorted(
                list(processed["employee_types"])
            )  # sorted for consistent output
        if "allowed_employee_groups" in processed and isinstance(
            processed["allowed_employee_groups"], set
        ):
            processed["allowed_employee_groups"] = sorted(
                list(processed["allowed_employee_groups"])
            )  # sorted
        return processed

    def _validate_contracted_hours(
        self, schedule_data: list[Schedule | dict[str, Any]]
    ) -> None:
        hours_by_employee = defaultdict(float)
        for entry_data in schedule_data:
            employee_id_val: int | None = None
            shift_obj: Any | None = None  # Could be ShiftTemplate object or None
            ...
            if isinstance(entry_data, dict):
                ...
                if shift_id and self.resources:  # Need resources to get shift template
                    ...
                    "duration_hours" in entry_data
                ):  # Maybe duration is directly in dict
        ...
        for emp_id, actual_hours in hours_by_employee.items():
            employee: ActualEmployee | None = None
            if TYPE_CHECKING:
                employee = self.resources.get_employee(
                    emp_id
                )  # Returns ActualEmployee or None
            ...
            try:
                ...
                if actual_hours < min_hours:
                    ...
                    if hasattr(employee, "first_name") and hasattr(
                        employee, "last_name"
                    ):
    ...
    def _validate_keyholders(
        ...
        for (shift_date, shift_id), employee_ids in shifts_by_date_shift.items():
            ...
            if not shift_template or not getattr(
            ...
            for emp_id in employee_ids:
                employee: ActualEmployee | None = None  # Hint for type checker
                if TYPE_CHECKING:
    ...
    def _validate_rest_periods(
        ...
        mock_entries = [
            entry for entry in schedule_data if hasattr(entry, "_mock_name")
        ]
        ...
        for entry_data in schedule_data:
            ...
            date_val = (
                entry_data.get("date")
                if isinstance(entry_data, dict)
                else getattr(entry_data, "date", None)
            )
            if date_val:
                entries_by_employee[employee_id_val].append(entry_obj_for_sort)
            else:
                logger.warning(
                    f"Skipping entry for rest period check due to missing date: {entry_data}"
                )
                ...
                sorted_entries = sorted(
                    ...
                    key=lambda e: (
                        ...
                        if isinstance(e, dict) and isinstance(e.get("date"), str)
                            ...
                            e, "date", date.min
                        )  # Fallback for sorting if not dict/str
                    ),
                )
            except (ValueError, AttributeError, TypeError):
                logger.warning(
                    f"Could not sort entries for employee {emp_id} for rest period check."
                )
                continue
            ...
            for i in range(len(sorted_entries) - 1):
                ...
                try:
                    ...
                    if rest_hours < self.config.min_rest_hours:
                        employee = self.resources.get_employee(emp_id)
                        employee_name = (
                            getattr(employee, "name", f"Employee {emp_id}")
                            if employee
                        ...
                        first_date_obj = (
                            first_entry.get("date")
                            if isinstance(first_entry, dict)
                            else getattr(first_entry, "date", None)
                        )
                        second_date_obj = (
                            second_entry.get("date")
                            if isinstance(second_entry, dict)
                            else getattr(second_entry, "date", None)
                        )

                        first_date_str = (
                            first_date_obj.isoformat()
                            if isinstance(first_date_obj, date)
                            else str(first_date_obj)
                        )
                        second_date_str = (
                            second_date_obj.isoformat()
                            if isinstance(second_date_obj, date)
                            else str(second_date_obj)
                        )
                    ...
                    logger.warning(
                        f"Could not calculate rest hours between {first_entry} and {second_entry}"
                    )
                    continue

    def _validate_max_shifts(
        self, schedule_data: list[Schedule | dict[str, Any]]
    ) -> None:
        ...
        for entry_data in schedule_data:
            ...
            entry_date_obj: date | None = None
            if isinstance(date_val, str):
                try:
                    entry_date_obj = datetime.fromisoformat(date_val).date()
                except ValueError:
                    continue
            elif isinstance(date_val, date):
        ...
        for emp_id, entries in shifts_by_employee_week.items():
            ...
            max_shifts = (
                self.config.max_shifts_per_group.get(emp_group, 5)
                if self.config.max_shifts_per_group
                else 5
            )
    ...
    def _validate_max_hours(
        ...
        for entry_data in schedule_data:
            emp_id: int | None = None
            date_val: date | str | None = None
            shift_id: int | None = None  # Need shift_id to get duration
            duration_val: float = 0.0
            ...
            if isinstance(entry_data, dict):
                ...
                if shift_obj:
                    if shift_id is None:
                        shift_id = getattr(shift_obj, "id", None)
                    try:
                        duration_val = float(getattr(shift_obj, "duration_hours", 0.0))
                    except (ValueError, TypeError):
                        duration_val = 0.0

            if (
                not emp_id or not date_val or shift_id is None
            ):  # Skip if basic info missing or no actual shift
                continue
            ...
            if duration_val == 0.0 and shift_id is not None:
                ...
                if shift_template:
                    try:
                        duration_val = float(
                            getattr(shift_template, "duration_hours", 0.0)
                        )
                    except (ValueError, TypeError):
                        duration_val = 0.0
        ...
        for emp_id, hours in hours_by_employee_week.items():
            ...
            emp_group = getattr(employee, "employee_group", None)
            max_hours = (
                self.config.max_hours_per_group.get(emp_group, 40)
                if self.config.max_hours_per_group
                else 40
            )
    ...
    def _calculate_rest_hours(
        self,
        first_entry: Schedule | dict[str, Any],
        second_entry: Schedule | dict[str, Any],
    ) -> float:
        ...
        if isinstance(first_entry, dict):
            ...
            first_end_time_str = first_entry.get("end_time") or (
                shift_data.get("end_time") if isinstance(shift_data, dict) else None
            )
        else:  # Object
            shift_obj = getattr(first_entry, "shift", None)
            first_end_time_str = getattr(first_entry, "end_time", None) or (
                getattr(shift_obj, "end_time", None) if shift_obj else None
            )
        ...
        if isinstance(second_entry, dict):
            ...
            second_start_time_str = second_entry.get("start_time") or (
                shift_data.get("start_time") if isinstance(shift_data, dict) else None
            )
        else:  # Object
            shift_obj = getattr(second_entry, "shift", None)
            second_start_time_str = getattr(second_entry, "start_time", None) or (
                getattr(shift_obj, "start_time", None) if shift_obj else None
            )
    ...
    def _validate_consecutive_days(
        ...
        for entry_data in schedule_data:
            emp_id: int | None = None
            date_val: date | str | None = None
            shift_id: int | None = None  # Check if shift exists
            ...
            if shift_id is not None and emp_id is not None and date_val is not None:
                ...
                employees_schedules[emp_id].append(
                    {"original": entry_data, "parsed_date": entry_date_obj}
                )
        ...
        for employee_id, entries_with_dates in employees_schedules.items():
            ...
            if max_consecutive > max_allowed:
                ...
                employee_name = (
                    getattr(employee, "name", f"Employee {employee_id}")
                    if employee
    ...
    def _validate_weekend_distribution(
        self, schedule_data: list[Schedule | dict[str, Any]]
    ) -> None:
    ...
    def _validate_early_late_rules(
        self, schedule_data: list[Schedule | dict[str, Any]]
    ) -> None:
        employees_schedules = defaultdict(list)
        for entry_data in schedule_data:
            ...
            if isinstance(entry_data, dict):
                ...
                if shift_obj:
                    ...
                    if start_time_str is None:
                        start_time_str = getattr(shift_obj, "start_time", None)
                    if end_time_str is None:
                        end_time_str = getattr(shift_obj, "end_time", None)
            ...
            for i in range(1, len(sorted_entries)):
                ...
                if (curr_entry["date"] - prev_entry["date"]).days == 1:
                    ...
                    if prev_end_str and curr_start_str:
                        ...
                        if isinstance(prev_end_str, str) and isinstance(
                            curr_start_str, str
                        ):
    ...
    def _validate_break_rules(
        ...
        for entry_data in schedule_data:
            ...
            date_val: date | str | None = None
            break_start_str: str | None = None
            break_end_str: str | None = None
            start_time_str: str | None = None
            end_time_str: str | None = None
                ...
                if not has_break:
                    ...
                    if isinstance(date_val, str):
                        try:
                            ...
                            entry_date_obj = None  # Or keep as string?
                    elif isinstance(date_val, date):
                        entry_date_obj = date_val

                    date_str_for_error = (
                        entry_date_obj.strftime("%Y-%m-%d")
                        if entry_date_obj
                        else str(date_val)
                    )
                    ...
                    employee = self.resources.get_employee(
                        employee_id
                    )  # employee_id is checked not None earlier
                    employee_name = (
                        f"{employee.first_name} {employee.last_name}"
                        if employee
                        and hasattr(employee, "first_name")
                        and hasattr(employee, "last_name")
                        else f"Employee {employee_id}"
                    )
    ...
    def get_coverage_summary(self) -> dict[str, Any]:
        ...
        min_employee_coverage_percent = (
            ...
            if self.total_intervals_checked > 0
            else 0
        )
        keyholder_coverage_percent = (
            (self.intervals_met_keyholder / self.intervals_needed_keyholder * 100)
            if self.intervals_needed_keyholder > 0
            else 100  # If none needed, it's 100% met
        )
