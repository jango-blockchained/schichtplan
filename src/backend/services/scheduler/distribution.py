...
class DistributionManager:
    ...
    def _get_shift_category_key(self, category: str, shift_template_like=None) -> str:
        ...
        if category == "EARLY": return "EARLY_MORNING"
        if category == "MIDDLE": return "STANDARD_DAY"
        if category == "LATE": return "LATE_EVENING"
        ...
        self.logger.warning(f"Unknown category '{category}' in _get_shift_category_key. Defaulting to STANDARD_DAY.")
    ...
    def calculate_assignment_score(
        ...
        elif avail_type_str == "AVAILABLE": score += 10.0
        else: 
        ...
        if target_interval_needs:
            if target_interval_needs.get("requires_keyholder"):
                if getattr(employee, 'is_keyholder', False): score += 150.0
                else: score -= 1000.0
            elif getattr(employee, 'is_keyholder', False): # Not required, but is keyholder
                 score -= 10.0 
            ...
            if required_target_employee_types:
                ...
                if employee_group and employee_group in required_target_employee_types: score += 120.0
                elif employee_group and employee_group not in required_target_employee_types: score -= 750.0
                elif not employee_group: score -= 100.0
        else:
        ...
        if contracted_hours_for_workload > 0 and current_hours_worked > contracted_hours_for_workload:
            workload_penalty_val += (current_hours_worked - contracted_hours_for_workload) * 1.5
        ...
        if shift_covered_intervals and full_day_staffing_snapshot:
            ...
            for interval_key in shift_covered_intervals:
                if interval_key in full_day_staffing_snapshot: 
                    ...
                    max_needed_staff = interval_staffing.get('max_needed', interval_staffing.get('required', 0))
                    if current_staff >= max_needed_staff:
                        num_covered_intervals_overstaffed += 1
            if num_covered_intervals_overstaffed > 0 and len(shift_covered_intervals) > 0 : 
                 overstaffing_penalty_val = (num_covered_intervals_overstaffed / len(shift_covered_intervals)) * 25.0 * self.fair_distribution_weight
        score -= overstaffing_penalty_val
        
        self.logger.debug(f"Final score for Emp {employee_id}, Shift {shift_template_id}, Date {shift_date}: {score}")
        return score
                ...
                if isinstance(assignment_dict, dict):
                    ...
                    if duration is None:
                        ...
                        if start_time_str and end_time_str:
                            try:
                                duration = self.calculate_duration(start_time_str, end_time_str)
                            except Exception as e:
                                self.logger.error(f"Error calculating duration in _get_employee_current_hours for emp {employee_id}: {e}")
