...
class DistributionManager:
    ...
    def distribute_shifts(self, start_date, end_date, preliminary_assignments):
        """
        Distribute shifts fairly among employees based on constraints and preferences.
        
        Args:
            start_date: The start date of the scheduling period
            end_date: The end date of the scheduling period
            preliminary_assignments: Initial assignment list generated by the scheduler
            
        Returns:
            A list of final assignments after applying distribution logic
        """
        self.logger.info(f"Starting shift distribution for period {start_date} to {end_date}")
        self.logger.info(f"Received {len(preliminary_assignments)} preliminary assignments")
        
        # Clear previous validation errors
        self.validation_errors = []
        
        # Group preliminary assignments by date for easier processing
        assignments_by_date = {}
        for assignment in preliminary_assignments:
            ...
            if isinstance(assignment, dict):
                ...
                assignment_date = getattr(assignment, "date", None)
                employee_id = getattr(assignment, "employee_id", None)
                shift_id = getattr(assignment, "shift_id", None)
                
            # Skip empty assignments or those without a date
            if not assignment_date or not shift_id:
                continue
        ...
        # Process assignments day by day
        self.logger.info(f"Processing {len(assignments_by_date)} days of assignments")
        
        # Initialize result list with assignments that we'll keep as-is
        final_assignments = []
        
        # Track current workload for each employee
        employee_workload = defaultdict(float)
        
        # Process one day at a time to maintain daily balance
        current_date = start_date
        while current_date <= end_date:
            if current_date in assignments_by_date:
                day_assignments = assignments_by_date[current_date]
                self.logger.info(f"Processing {len(day_assignments)} assignments for {current_date}")
                
                # Apply constraints and check validity
                valid_assignments = []
                for assignment in day_assignments:
                    # Validate assignment against constraints if constraint checker is available
                    is_valid = True
                    if self.constraint_checker:
                        # Get relevant data depending on whether assignment is dict or object
                        if isinstance(assignment, dict):
                            emp_id = assignment.get("employee_id")
                            shift_id = assignment.get("shift_id")
                        else:
                            emp_id = getattr(assignment, "employee_id", None)
                            shift_id = getattr(assignment, "shift_id", None)
                            
                        if emp_id and shift_id:
                            ...
                            if employee and shift:
                                ...
                                if violations:
                                    self.logger.warning(
                                        f"Assignment violates constraints: {violations}"
                                    )
                                    error_msg = f"Employee {emp_id} cannot work shift {shift_id} on {current_date}: {'; '.join(violations)}"
                ...
                # Add valid assignments to the final list
                final_assignments.extend(valid_assignments)
                ...
                for assignment in valid_assignments:
                    ...
                    if emp_id and shift_id:
                        # Get shift to determine duration
                        shift = self.resources.get_shift(shift_id)
                        if shift and hasattr(shift, "duration_hours") and shift.duration_hours:
                            # Add shift duration to employee workload
                            employee_workload[emp_id] += shift.duration_hours
            
            # Move to next day
            current_date += timedelta(days=1)
        
        # Add any remaining assignments that weren't processed by date
        for assignment in preliminary_assignments:
            # Determine if assignment is already in final_assignments
            is_duplicate = False
            for final_assignment in final_assignments:
                if (isinstance(assignment, dict) and isinstance(final_assignment, dict)):
                    if (assignment.get("employee_id") == final_assignment.get("employee_id") and
                        assignment.get("shift_id") == final_assignment.get("shift_id") and
                        assignment.get("date") == final_assignment.get("date")):
                        is_duplicate = True
                        break
                        
            if not is_duplicate:
                # Check if this is an empty entry (without shift_id)
                is_empty = False
                if isinstance(assignment, dict):
                    is_empty = assignment.get("shift_id") is None or assignment.get("is_empty", False)
                else:
                    is_empty = getattr(assignment, "shift_id", None) is None or getattr(assignment, "is_empty", False)
                    
                if is_empty:
                    # Keep empty entries as they are
                    final_assignments.append(assignment)
        
        # Log results of distribution
        self.logger.info(f"Distribution completed with {len(final_assignments)} final assignments")
        if self.validation_errors:
            self.logger.warning(f"Found {len(self.validation_errors)} validation errors during distribution")
            
        # Log workload summary
        if employee_workload:
            for emp_id, hours in employee_workload.items():
                self.logger.info(f"Employee {emp_id} assigned {hours:.1f} total hours")
                
        return final_assignments
        
    def get_validation_errors(self):
        """Return any validation errors from the distribution process"""
