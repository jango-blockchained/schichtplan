...
class MEPPDFGenerator:
    ...
    def generate_mep_pdf(
        ...
        total_pages = max(1, (total_employees + self.EMPLOYEES_PER_PAGE - 1) // self.EMPLOYEES_PER_PAGE)
    ...
    def _build_page_content(
        ...
        if page_employees:
            table = self._build_main_table(page_employees, processed_data['date_range_days'])
    ...
    def _build_main_table(
        self,
        employee_schedules: Dict[int, Dict],
        ...
        for day_info in date_range_days:
            ...
            header_row1.append(Paragraph(f"{day_name}<br/>{day_date}", self.table_cell_style))
        ...
        for emp_id, emp_data in employee_schedules.items():
            ...
            # Employee row
            emp_row = [
                Paragraph(f"{employee_info['first_name']}<br/>{employee_info['last_name']}", self.table_cell_style),
        ...
        max_employees_on_page = min(self.EMPLOYEES_PER_PAGE, 4)  # Limit to 4 employees for testing
    ...
    def _calculate_column_widths(self, num_days: int) -> List[float]:
        ...
        fixed_width = name_width + function_width + plan_width + weekly_width + monthly_width
    ...
    def _build_footer(self) -> List:
        ...
        pause_text = (
            "Pausenzeiten: bis 6 Stunden : keine Pause, "
    ...
    def _get_page_employees(self, employee_schedules: Dict[int, Dict], page_num: int) -> Dict[int, Dict]:
