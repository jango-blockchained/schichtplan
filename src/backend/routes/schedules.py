...
def get_versions_for_date_range(start_date, end_date):
    ...
    try:
        ...
        if versions:
            logger.debug(f"Found {len(versions)} versions in version_meta")
            return versions
        ...
        logger.debug(f"Found {len(version_numbers)} versions in schedules table")
        ...
        for (version,) in version_numbers:
            try:
                ...
                if not meta:
                    logger.debug(f"Creating metadata for version {version}")
                    ...
                    if dates:
                        ...
                        try:
                            db.session.commit()
                            logger.debug(f"Created metadata for version {version}")
                        except Exception as e:
                            db.session.rollback()
                            logger.error(
        ...
        if not result:
            ...
            logger.debug(f"Successfully processed {len(result)} versions")
...
def get_schedules():
    ...
    try:
        ...
        if version is None:
            ...
            logger.info(f"Using latest version for date range: {version}")
        ...
        placeholder_shift = (
            db.session.query(ShiftTemplate)
            .filter_by(start_time="00:00", end_time="00:00")
            .first()
        )
        ...
        versions_list = [
            {
                ...
                "date_range_start": v.date_range_start.isoformat()
                if v.date_range_start
                else None,
                "date_range_end": v.date_range_end.isoformat()
                if v.date_range_end
                else None,
                "notes": v.notes,
            }
...
def create_schedule_entry():
    ...
    try:
        ...
        if (
            request_data.employee_id is None
            or request_data.date is None
            or request_data.version is None
        ):
            return jsonify(
                {
                    "status": "error",
                    "message": "Missing required fields for creating a new schedule: employee_id, date, and version",
                }
            ), HTTPStatus.BAD_REQUEST
        ...
        if (
            ...
            try:
                ...
                logger.error(
                    f"Error calculating break duration for new schedule {schedule.id}: {str(e)}"
                )
        ...
        return jsonify(
            {
                "status": "error",
                "message": "Invalid input for creating schedule entry.",
                "details": e.errors(),
            }
        ), HTTPStatus.BAD_REQUEST
        ...
        db.session.rollback()
        logger.error(
            f"Database integrity error creating schedule entry: {str(e)}", exc_info=True
        )
        return jsonify(
            {
                "status": "error",
                "message": "Database error creating schedule entry.",
                "details": "A schedule entry for this employee, date, and version might already exist.",
            }
        ), HTTPStatus.CONFLICT  # Use 409 for conflict
    except Exception as e:
        db.session.rollback()
        logger.error(
            f"An unexpected error occurred creating schedule entry: {str(e)}",
            exc_info=True,
        )
        return jsonify(
            {
                "status": "error",
                "message": "An internal server error occurred while creating schedule entry.",
                "details": str(e),
            }
        ), HTTPStatus.INTERNAL_SERVER_ERROR
...
def generate_schedule():
    ...
    try:
        ...
        generator = (
            ScheduleGenerator()
        )  # Assuming the generator is designed to work within a Flask app context
        ...
        logger.error(
            f"Schedule resource error during generation: {str(e)}", exc_info=True
        )
        return jsonify(
            {"status": "error", "message": f"Failed during resource loading: {str(e)}"}
        ), HTTPStatus.INTERNAL_SERVER_ERROR
    except Exception as e:
        logger.error(
            f"An unexpected error occurred during schedule generation: {str(e)}",
            exc_info=True,
        )
...
def update_schedule(schedule_id):
    ...
    try:
        ...
        if schedule_id == 0:
            ...
            if request_data.employee_id is None or request_data.date is None:
                return jsonify(
                    {
                        "status": "error",
                        "message": "Missing required fields for creating a new schedule: employee_id and date",
                    }
                ), HTTPStatus.BAD_REQUEST
            ...
            if request_data.shift_id is not None or hasattr(request_data, "shift_id"):
                ...
                if request_data.shift_id is None:
                    ...
                    logger.info(f"After update, shift_id={schedule.shift_id}")
            ...
            if request_data.version is not None:
                ...
                logger.info(f"Updated schedule version to {schedule.version}")

            if (
                request_data.availability_type is not None
            ):  # Check if explicitly provided
                # Handle availability_type conversion correctly
                schedule.availability_type = AvailabilityType(
                    request_data.availability_type
                )
            elif hasattr(
                schedule, "availability_type"
            ):  # If not provided, ensure it has a default or remains None if allowed by model
                schedule.availability_type = (
                    None  # Assuming None is allowed if not provided in update
                )
                logger.info(
        ...
        if schedule_id > 0 and (
            request_data.shift_id is None and hasattr(request_data, "shift_id")
        ):
            logger.warning(
                f"DELETION OPERATION COMPLETED - Schedule {schedule_id} now has shift_id={schedule.shift_id}"
            )
        else:
            logger.info(f"Database commit successful for schedule_id={schedule_id}")
...
def publish_schedule(version):
    try:
        ...
        db.session.commit()
        return jsonify(
            {"status": "success", "message": "Schedule published successfully"}
        )
    except Exception as e:
        db.session.rollback()
...
def archive_schedule(version):
    try:
        ...
        db.session.commit()
        return jsonify(
            {"status": "success", "message": "Schedule archived successfully"}
        )
    except Exception as e:
        db.session.rollback()
...
def validate_schedule():
    ...
    try:
        ...
        if not schedule_ids:
            return jsonify(
                {"status": "error", "message": "No schedule IDs provided"}
            ), HTTPStatus.BAD_REQUEST
        ...
        if not schedules:
            return jsonify(
                {"status": "error", "message": "No schedules found"}
            ), HTTPStatus.NOT_FOUND
        ...
        return jsonify(
            {"status": "error", "message": f"Resource error: {str(e)}"}
        ), HTTPStatus.BAD_REQUEST
    except Exception as e:
...
def duplicate_version():
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Could not create version metadata: {str(e)}")
    ...
    try:
        ...
        if not compare_schedules:
            return jsonify(
                {
                    "status": "error",
                    "message": f"Compare version {compare_version} not found",
                }
            ), HTTPStatus.NOT_FOUND
...
def update_version_notes(version):
    ...
    try:
        ...
        if notes is None:
            return jsonify(
                {"status": "error", "message": "notes field is required"}
            ), HTTPStatus.BAD_REQUEST
        ...
        try:
            ...
            return jsonify(
                {
                    "status": "error",
                    "message": f"Failed to update version notes: {str(e)}",
                }
            ), HTTPStatus.INTERNAL_SERVER_ERROR
...
def fix_schedule_display():
    ...
    try:
        ...
        incomplete_schedules = [
            s
            for s in schedules
            if s.shift_id is not None and (not hasattr(s, "shift") or s.shift is None)
        ]
        logger.info(
            f"{len(incomplete_schedules)} schedules have shift_id but missing relationship data"
        )
        ...
        if not default_shift:
            return jsonify(
                {"status": "error", "message": "No default shift found to assign"}
            ), HTTPStatus.BAD_REQUEST
        ...
        for date_obj in [
            ...
            if not date_with_shifts:
                ...
                if first_schedule:
                    ...
                    if hasattr(default_shift, "shift_type_id") and hasattr(
                        first_schedule, "shift_type_id"
                    ):
                    ...
                    if not first_schedule.status or first_schedule.status not in [
                    ...
                    logger.info(
                        f"Assigned shift {default_shift.id} to employee {first_schedule.employee_id} on {date_obj}"
                    )

                    # Also check other schedules on this day that might need shift_id update
                    for other_schedule in date_schedules:
                        if (
                            other_schedule.shift_id is None
                            and other_schedule != first_schedule
                        ):
                            # Maybe assign shifts to a few more employees on this day
                            if (
                                len(days_fixed) < 3 and assigned_count < 10
                            ):  # Limit how many we fix
                                other_schedule.shift_id = default_shift.id
                                other_schedule.shift = (
                                    default_shift  # Set relationship explicitly
                                )
        ...
        if incomplete_schedules:
            for schedule in incomplete_schedules:
                ...
                if schedule.shift_id in shift_lookup:
                    ...
                    logger.info(
                        f"Fixed relationship for schedule {schedule.id} with shift {schedule.shift_id}"
                    )
                    assigned_count += 1

                    # Set shift_type_id if available
                    if hasattr(
                        shift_lookup[schedule.shift_id], "shift_type_id"
                    ) and hasattr(schedule, "shift_type_id"):
                        schedule.shift_type_id = shift_lookup[
                            schedule.shift_id
                        ].shift_type_id
        ...
        if schedules_with_shifts:
            ...
            has_shift_data = (
                hasattr(sample_schedule, "shift") and sample_schedule.shift is not None
            )
            logger.info(
                f"Sample schedule (ID={sample_schedule.id}) has shift data: {has_shift_data}"
            )
            if has_shift_data:
                logger.info(
                    f"Shift details: start={sample_schedule.shift.start_time}, end={sample_schedule.shift.end_time}"
                )
        ...
        for schedule in updated_schedules:
            ...
            if schedule.shift_id is not None and (
                "shift_start" not in schedule_dict or "shift_end" not in schedule_dict
            ):
                # Try to get the shift from our lookup
                if schedule.shift_id in shift_lookup:
                    shift = shift_lookup[schedule.shift_id]
        ...
        return jsonify(
            {
                ...
                "sample_schedule": sample_schedule.to_dict()
                if sample_schedule
                else None,
    ...
    try:
        ...
        try:
            ...
            return jsonify(
                {
                    "status": "error",
                    "message": "Invalid or missing date format, expected YYYY-MM-DD",
                }
            ), HTTPStatus.BAD_REQUEST
        ...
        with current_app.app_context():
            ...
            logger.info(
                f"Fetched {len(all_availabilities)} employee availability entries (including recurring)."
            )
        ...
        logger.error(
            "Invalid date format received for AI schedule generation", exc_info=True
        )
        return jsonify(
