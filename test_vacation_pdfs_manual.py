...
class MockEmployee:
    def __init__(self, id, first_name, last_name, employee_id, vacation_per_year=30, email=None):
        self.id = id
...
class MockAbsence:
    def __init__(self, id, employee_id, absence_type_id, start_date, end_date, status, note=None):
        self.id = id
        self.employee_id = employee_id
...
def main():
    ...
    absences = [
        MockAbsence(1, 1, "vacation", datetime(year, 7, 1).date(), datetime(year, 7, 14).date(), "approved", "Sommerurlaub"),
        MockAbsence(2, 2, "vacation", datetime(year, 8, 1).date(), datetime(year, 8, 7).date(), "requested", "Urlaub"),
        MockAbsence(3, 3, "vacation", datetime(year, 7, 15).date(), datetime(year, 7, 21).date(), "approved", None),
        MockAbsence(4, 4, "vacation", datetime(year, 9, 1).date(), datetime(year, 9, 10).date(), "approved", "Herbsturlaub"),
        MockAbsence(5, 1, "vacation", datetime(year, 12, 23).date(), datetime(year, 12, 30).date(), "requested", "Weihnachten"),
    ]
    ...
    try:
        ...
        output_path = output_dir / f"employee_request_{employees[0].employee_id}_filled.pdf"
        with open(output_path, 'wb') as f:
            f.write(pdf_buffer.read())
        print(f"   ✓ Saved: {output_path}")
    except Exception as e:
    ...
    try:
        ...
        output_path = output_dir / f"employee_request_{employees[1].employee_id}_blank.pdf"
        with open(output_path, 'wb') as f:
            f.write(pdf_buffer.read())
        print(f"   ✓ Saved: {output_path}")
    except Exception as e:
