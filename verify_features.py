...
def check_settings():
    ...
    with app.app_context():
        ...
        if settings.week_month_boundary_mode == "split_by_month":
            ...
            print(
                "  ℹ️  Split week mode is DISABLED (currently: {})".format(
                    settings.week_month_boundary_mode
                )
            )
            print(
                "     To enable, set week_month_boundary_mode to 'split_by_month' in settings"
            )
...
def check_availability_data():
    ...
    with app.app_context():
        ...
        employees_with_availability = (
            db.session.query(EmployeeAvailability.employee_id).distinct().all()
        )
        total_employees = Employee.query.count()

        print(f"Total availability records: {len(availabilities)}")
        print(
            f"Employees with availability data: {len(employees_with_availability)} / {total_employees}"
        )

        if len(availabilities) == 0:
            print("\n⚠️  No availability data found")
            print(
                "   The hover modal will show 'Keine Verfügbarkeiten' for all employees"
            )
            print(
                "   To test with data, use the Enhanced Availability Modal to add availabilities"
            )
        else:
            ...
            if len(availabilities) > 0:
                ...
                for avail in availabilities[:5]:
                    ...
                    day_name = (
                        ...
                        if 0 <= avail.day_of_week < 7
                        else "Unknown"
                    )
                    print(
                        f"  - {employee_name} ({day_name} {avail.hour:02d}:00): {avail.availability_type} "
...
def check_week_segments():
    ...
    with app.app_context():
        ...
        if week_info.spans_months:
            ...
            # Test split mode
            segments_split = get_week_segments(
                week_info, MonthBoundaryMode.SPLIT_ON_MONTH
            )
            print(f"\n  Split mode segments: {len(segments_split)}")
            for seg in segments_split:
                print(
                    f"    - Segment {seg.segment_number}/{seg.total_segments}: "
