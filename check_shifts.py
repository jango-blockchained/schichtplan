...
with app.app_context():
    ...
    for shift in resources.shifts:
        ...
        if shift_type:
            print(f'ID: {shift.id}, Type: {shift_type}, Hours: {shift.start_time}-{shift.end_time}')
        else:
            ...
            print(f'ID: {shift.id}, Explicit Type: None, Inferred Type: {category}, Hours: {shift.start_time}-{shift.end_time}')
    ...
    for shift_type in ["EARLY", "MIDDLE", "LATE"]:
        ...
        print(f'Shifts of type {shift_type}: {len(type_shifts)} ({[s.id for s in type_shifts]})')
    
    print('-' * 80)
    print('Shifts by time of day (based on start hour):')
    
    early_shifts = [s for s in resources.shifts if int(s.start_time.split(':')[0]) < 10]
    middle_shifts = [s for s in resources.shifts if 10 <= int(s.start_time.split(':')[0]) < 14]
    late_shifts = [s for s in resources.shifts if int(s.start_time.split(':')[0]) >= 14]
    
    print(f'Morning shifts (before 10:00): {len(early_shifts)} ({[s.id for s in early_shifts]})')
    print(f'Midday shifts (10:00-13:59): {len(middle_shifts)} ({[s.id for s in middle_shifts]})')
    print(f'Evening shifts (14:00+): {len(late_shifts)} ({[s.id for s in late_shifts]})')
