...
def test_user_model():
    ...
    try:
        ...
        assert all(len(code) == 12 for code in codes), "Each code should be 12 characters"
        print(f"✓ Generated {len(codes)} recovery codes")
        
        # Test recovery code verification
        test_code = codes[0]
        assert user.verify_recovery_code(test_code), "Should verify correct recovery code"
        assert not user.verify_recovery_code(test_code), "Should not verify same code twice"
        ...
        assert "remaining_recovery_codes" in user_dict, "to_dict should include remaining_recovery_codes"
...
def test_settings_model():
    ...
    try:
        ...
        assert hasattr(settings, 'initial_setup_completed'), "Settings should have initial_setup_completed field"
        ...
        print(f"✓ Default settings created (initial_setup_completed: {default_settings.initial_setup_completed})")
