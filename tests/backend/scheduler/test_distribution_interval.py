...
def create_mock_employee(
    id, is_keyholder=False, employee_group=None, contracted_hours=40.0, is_active=True
):
    mock_emp = MagicMock()
    mock_emp.id = id
...
def create_mock_shift_template(
    id, start_time_str, end_time_str, shift_type="GENERAL", duration_hours=None
):
    mock_st = MagicMock()
    mock_st.id = id
    ...
    mock_st.shift_type_id = (
        shift_type  # Assuming shift_type can serve as shift_type_id for categorization
    )
...
def common_mocks(app):
    ...
    employee1 = create_mock_employee(
        id=1, is_keyholder=True, employee_group="Supervisor"
    )
    employee2 = create_mock_employee(id=2, is_keyholder=False, employee_group="Cashier")
    employee3 = create_mock_employee(
        id=3, is_keyholder=False, employee_group="Cashier", contracted_hours=20
    )
    employee_untyped = create_mock_employee(
        id=4, is_keyholder=False, employee_group=None
    )

    mock_resources.employees = [employee1, employee2, employee3, employee_untyped]
    mock_resources.get_employee_by_id.side_effect = lambda eid: next(
        (e for e in mock_resources.employees if e.id == eid), None
    )
    mock_resources.get_employee = mock_resources.get_employee_by_id

    shift_template_early = create_mock_shift_template(
        id=101, start_time_str="08:00", end_time_str="16:00", shift_type="EARLY"
    )
    shift_template_late = create_mock_shift_template(
        id=102, start_time_str="14:00", end_time_str="22:00", shift_type="LATE"
    )
    shift_template_short = create_mock_shift_template(
        id=103, start_time_str="10:00", end_time_str="14:00", shift_type="MIDDLE"
    )

    mock_resources.shift_templates = [
        shift_template_early,
        shift_template_late,
        shift_template_short,
    ]
    mock_resources.get_shift_template_by_id.side_effect = lambda sid: next(
        (st for st in mock_resources.shift_templates if st.id == sid), None
    )
...
def test_calculate_score_overstaffing_penalty(common_mocks):
    ...
    mock_resources.get_interval_needs_for_time.side_effect = (
        lambda dt, t: context_no_overstaff["full_day_staffing_snapshot"][t]
    )
    ...
    mock_resources.get_interval_needs_for_time.side_effect = (
        lambda dt, t: context_causes_overstaff["full_day_staffing_snapshot"][t]
    )
...
def test_try_find_and_make_assignment_success(common_mocks):
    pytest.skip(
        "_try_find_and_make_assignment does not exist in current DistributionManager implementation."
    )
...
def test_perform_interval_based_assignments_simple_case(mock_get_needs, common_mocks):
    pytest.skip(
        "update_with_assignment does not exist in current DistributionManager implementation."
    )


@patch("src.backend.services.scheduler.distribution.get_required_staffing_for_interval")
def test_perform_interval_based_assignments_no_assignments_possible(
    mock_get_needs, common_mocks
):
    pytest.skip(
        "update_with_assignment does not exist in current DistributionManager implementation."
    )
...
@patch("src.backend.services.scheduler.distribution.get_required_staffing_for_interval")
def test_perform_interval_based_assignments_multiple_staff_needed_for_interval(
    mock_get_needs, common_mocks
):
    pytest.skip(
        "update_with_assignment does not exist in current DistributionManager implementation."
